...
def _g_sup_rr_r(params, transforms, profiles, data, **kwargs):
    data["g^rr_r"] = (
        ...
        * dot(data["e^rho"], cross(data["e_rho"], data["e_theta_r"]))
        + data["sqrt(g)_r"]
        / data["sqrt(g)"] ** 2
        * dot(data["e^rho"], cross(data["e_theta"], data["e_rho"]))
        + 1
        / data["sqrt(g)"]
        * dot(data["e^rho"], cross(data["e_theta_r"], data["e_rho"]))
        + 1
        / data["sqrt(g)"]
        * dot(data["e^rho"], cross(data["e_theta"], data["e_rho_r"]))
    )
    return data
...
def _g_sup_rt_r(params, transforms, profiles, data, **kwargs):
    data["g^rt_r"] = (
        ...
        * dot(data["e^theta"], cross(data["e_theta_r"], data["e_zeta"]))
        + 1
        / data["sqrt(g)"]
        * dot(data["e^theta"], cross(data["e_theta"], data["e_zeta_r"]))
        + data["sqrt(g)_r"]
        / data["sqrt(g)"] ** 2
...
def _g_sup_rz_r(params, transforms, profiles, data, **kwargs):
    data["g^rz_r"] = (
        data["sqrt(g)_r"]
        / data["sqrt(g)"] ** 2
        * dot(data["e^zeta"], cross(data["e_zeta"], data["e_theta"]))
        + 1
        / data["sqrt(g)"]
        * dot(data["e^zeta"], cross(data["e_zeta_r"], data["e_theta"]))
        + 1
        / data["sqrt(g)"]
        * dot(data["e^zeta"], cross(data["e_zeta"], data["e_theta_r"]))
        + data["sqrt(g)_r"]
        / data["sqrt(g)"] ** 2
        * dot(data["e^rho"], cross(data["e_theta"], data["e_rho"]))
        + 1
...
def _g_sup_tt_r(params, transforms, profiles, data, **kwargs):
    data["g^tt_r"] = (
        data["sqrt(g)_r"]
        / data["sqrt(g)"] ** 2
        * dot(data["e^theta"], cross(data["e_theta"], data["e_rho"]))
        + 1
        / data["sqrt(g)"]
        * dot(data["e^theta"], cross(data["e_theta_r"], data["e_rho"]))
        + 1
        / data["sqrt(g)"]
        * dot(data["e^theta"], cross(data["e_theta"], data["e_rho_r"]))
        + data["sqrt(g)_r"]
        / data["sqrt(g)"] ** 2
...
def _g_sup_zz_r(params, transforms, profiles, data, **kwargs):
    data["g^zz_r"] = (
        ...
        * dot(data["e^zeta"], cross(data["e_zeta"], data["e_rho_r"]))
        + data["sqrt(g)_r"]
        / data["sqrt(g)"] ** 2
        * dot(data["e^zeta"], cross(data["e_rho"], data["e_zeta"]))
        + 1
        / data["sqrt(g)"]
        * dot(data["e^zeta"], cross(data["e_rho_r"], data["e_zeta"]))
        + 1
        / data["sqrt(g)"]
...
def _g_sup_rr_t(params, transforms, profiles, data, **kwargs):
    data["g^rr_t"] = (
        ...
        * dot(data["e^rho"], cross(data["e_theta"], data["e_rho"]))
        + 1
        / data["sqrt(g)"]
        * dot(data["e^rho"], cross(data["e_theta_t"], data["e_rho"]))
        + 1
        / data["sqrt(g)"]
        * dot(data["e^rho"], cross(data["e_theta"], data["e_rho_t"]))
    )
    return data
...
def _g_sup_rz_t(params, transforms, profiles, data, **kwargs):
    data["g^rz_t"] = (
        ...
        * dot(data["e^zeta"], cross(data["e_zeta"], data["e_theta"]))
        + 1
        / data["sqrt(g)"]
        * dot(data["e^zeta"], cross(data["e_zeta_t"], data["e_theta"]))
        + 1
        / data["sqrt(g)"]
        * dot(data["e^zeta"], cross(data["e_zeta"], data["e_theta_t"]))
        + data["sqrt(g)_t"]
        / data["sqrt(g)"] ** 2
        * dot(data["e^rho"], cross(data["e_theta"], data["e_rho"]))
        + 1
...
def _g_sup_tt_t(params, transforms, profiles, data, **kwargs):
    data["g^tt_t"] = (
        ...
        * dot(data["e^theta"], cross(data["e_theta"], data["e_rho"]))
        + 1
        / data["sqrt(g)"]
        * dot(data["e^theta"], cross(data["e_theta_t"], data["e_rho"]))
        + 1
        / data["sqrt(g)"]
        * dot(data["e^theta"], cross(data["e_theta"], data["e_rho_t"]))
        + data["sqrt(g)_t"]
        / data["sqrt(g)"] ** 2
        * dot(data["e^theta"], cross(data["e_rho"], data["e_theta"]))
        + 1
...
def _g_sup_zz_t(params, transforms, profiles, data, **kwargs):
    data["g^zz_t"] = (
        ...
        * dot(data["e^zeta"], cross(data["e_rho"], data["e_zeta"]))
        + 1
        / data["sqrt(g)"]
        * dot(data["e^zeta"], cross(data["e_rho_t"], data["e_zeta"]))
        + 1
        / data["sqrt(g)"]
...
def _g_sup_rr_z(params, transforms, profiles, data, **kwargs):
    data["g^rr_z"] = (
        ...
        * dot(data["e^rho"], cross(data["e_theta"], data["e_rho"]))
        + 1
        / data["sqrt(g)"]
        * dot(data["e^rho"], cross(data["e_theta_z"], data["e_rho"]))
        + 1
        / data["sqrt(g)"]
        * dot(data["e^rho"], cross(data["e_theta"], data["e_rho_z"]))
    )
    return data
...
def _g_sup_rz_z(params, transforms, profiles, data, **kwargs):
    data["g^rz_z"] = (
        ...
        * dot(data["e^zeta"], cross(data["e_zeta"], data["e_theta"]))
        + 1
        / data["sqrt(g)"]
        * dot(data["e^zeta"], cross(data["e_zeta_z"], data["e_theta"]))
        + 1
        / data["sqrt(g)"]
        * dot(data["e^zeta"], cross(data["e_zeta"], data["e_theta_z"]))
        + data["sqrt(g)_z"]
        / data["sqrt(g)"] ** 2
        * dot(data["e^rho"], cross(data["e_theta"], data["e_rho"]))
        + 1
...
def _g_sup_tt_z(params, transforms, profiles, data, **kwargs):
    data["g^tt_z"] = (
        ...
        * dot(data["e^theta"], cross(data["e_theta"], data["e_rho"]))
        + 1
        / data["sqrt(g)"]
        * dot(data["e^theta"], cross(data["e_theta_z"], data["e_rho"]))
        + 1
        / data["sqrt(g)"]
        * dot(data["e^theta"], cross(data["e_theta"], data["e_rho_z"]))
        + data["sqrt(g)_z"]
        / data["sqrt(g)"] ** 2
        * dot(data["e^theta"], cross(data["e_rho"], data["e_theta"]))
        + 1
...
def _g_sup_zz_z(params, transforms, profiles, data, **kwargs):
    data["g^zz_z"] = (
        ...
        * dot(data["e^zeta"], cross(data["e_rho"], data["e_zeta"]))
        + 1
        / data["sqrt(g)"]
        * dot(data["e^zeta"], cross(data["e_rho_z"], data["e_zeta"]))
        + 1
        / data["sqrt(g)"]
