...
def array_flatten_op(arr1, arr2):
    """Flatten values in each nonzero grouping in the array to the min group value.

    Get a modified version of arr1 where each well (group of nonzero values)
    is changed to the value with the index corresponding to the minimum value
    along arr2 in that well
...
def seg_argmin_op(arr1, arr2, start, end):
    """Get min index of values in arr2 per nonzero group in arr 1.

    Get indices of minimum values in arr2 in each segment of arr1
    (group of nonzero values)
