...
Phi = phi.reshape(L, N_surf)
Theta = theta.reshape(L, N_surf)
Pho = pho.reshape(L, N_surf)

fig,axs = plt.subplots(3,6,figsize=(14,8), sharex=True, sharey=True)

for i,a in enumerate(axs.flatten()):
    #j = np.argwhere(Pho[i] != 0)[:,0]
    j = Pho[i] != 0
    a.scatter(Phi[i,j], Theta[i,j] % (2*np.pi) , c=Pho[i,j], s=3, cmap='coolwarm')
    a.set_title(i-3)
