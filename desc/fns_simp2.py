...
def _compute_magnetic_field_from_Current(Kgrid,
    ...
    def nfp_loop(j, f):
        ...
        phi = phi[:, None]  # shape (num_nodes, 1) for broadcasting

        # Replace phi column in _rs with updated phi
        rs = jnp.concatenate( ( _rs[:, [0]], phi, _rs[:, [2]] ), axis=1 )
        rs = rpz2xyz(rs)  # shape (num_nodes, 3)
        rs = rs[:, None, :]
        #K = rpz2xyz_vec(_K, phi=phi)  # shape compatible with _K
        #K = rpz2xyz_vec(_K, phi=phi[:, 0])
        K = rpz2xyz_vec(_K, phi=phi[:, None, 0])
