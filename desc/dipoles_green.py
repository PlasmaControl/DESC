...
def normal_flux(sdata1,
    ...
    _b_pol_dips, _b_tor_dips = B_dips_vec(sdata1,
                                         sgrid, surface, 
                                         N, d_0, 
                                         coords,
                                         du_data, dd_data, dr_data, dl_data,
    ...
    _b_pol_sticks, _b_tor_sticks = B_sticks_vec(sgrid,
                                                coords,
                                                 du_data, dd_data, dr_data, dl_data,
                                                 index_i,index_f,
                                                 )
    ...
    # Dot at each point for each dipole: (npts, Ndips)
    Bn_pol = jnp.sum(B_pol_ * n_rho[:, :, None], axis=1)
    Bn_tor = jnp.sum(B_tor_ * n_rho[:, :, None], axis=1)

    return Bn_pol, Bn_tor
...
def B_sticks_vec(sgrid,
    ...
    jnp.save('b_stick_ij_pol_indexi_' + str(index_i) + '_indexi_' + str(index_f) + '_.npy', b_stick_pol)
    jnp.save('b_stick_ij_tor_indexi_' + str(index_i) + '_indexi_' + str(index_f) + '_.npy', b_stick_tor)
...
def K_dips_vec(sdata1,
               sgrid, surface,
               du_data, dd_data, dr_data, dl_data,
               N, d_0,
               index_i,index_f,):
    
    # Initialte the omegas
    ...
    for i in range(1, sgrid.NFP):
        ...
        G_w_pol_total += G_w_pair(sdata1,
                                du_data["u_iso"][index_i:index_f] + w_shift * i, du_data["v_iso"][index_i:index_f],
                                dd_data["u_iso"][index_i:index_f] + w_shift * i, dd_data["v_iso"][index_i:index_f],
                                N, d_0)
        
        G_w_tor_total += G_w_pair(sdata1,
                               dr_data["u_iso"][index_i:index_f] + w_shift * i, dr_data["v_iso"][index_i:index_f],
                               dl_data["u_iso"][index_i:index_f] + w_shift * i, dl_data["v_iso"][index_i:index_f],
                               N, d_0)
    ...
    jnp.save('Kij_pol_indexi_' + str(index_i) + '_indexi_' + str(index_f) + '_.npy', K_pol_total)
    jnp.save('Kij_tor_indexi_' + str(index_i) + '_indexi_' + str(index_f) + '_.npy', K_tor_total)
