...
def GammaC(name, s, nfulltransits, stepswithin1FP, bpstep, alpha = 0):
    ...
    dVdb_t1 = data["iota_r"] * dot(cross(grad_psi, data["B"]), grad_zeta) / B
    ...
    def argmin_loop(arr, start, end):
        """Find the argmin of an array between arr[start] and arr[end]"""
        def body(i, amin):
            return jnp.where(arr[i] < arr[amin], i, amin)
    
        return jax.lax.fori_loop(start, end, body, start)

    def body(i, wellGamma_c):
        min_B_ind = argmin_loop(B_array, start[i], end[i])
    ...
    wellGamma_c = jax.lax.fori_loop(0, len(start), body, wellGamma_c)
