...
class ChebyshevBasisSet:
    ...
    def intersect1d(self, k=0.0, num_intersect=None, pad_value=0.0):
        ...
        is_z2 = (df_dy_sign >= 0) & fix_inversion(is_intersect, df_dy_sign)
    ...
    def plot1d(
        ...
        k_transparency=0.5,
        num=1000,
        title=r"Intersects $z$ for $f(z) - k = 0$",
        hlabel=r"$z$",
        vlabel=r"$f(z)$",
        show=True,
    ):
        """Plot the function ``f`` defined by the Chebyshev coefficients.
        ...
        add2legend(legend, ax.plot(z, self.eval1d(z, cheb), label=vlabel))
        plot_intersect(ax, legend, z1, z2, k, k_transparency)
...
class Bounce2D:
    ...
    Notes
    -----
    Motivation and description of algorithm for developers.
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self._x, self._w = get_quad(quad, automorphism)
    ...
    def reshape_data(grid, data, names):
        ...
        names : list[str]
            Strings of keys in ``data`` to reshape.
    ...
    def integrate(self, pitch, integrand, f, weight=None, num_well=None):
        ...
        num_well : int or None
            ...
            which enumerates bounce integrals for a particular field line and pitch,
            is padded with zero.
            ...
            tightly bounds the actual number of wells. As a reference, there are
            typically at most 5 wells per toroidal transit for a given pitch.
...
class Bounce1D:
    ...
    def __init__(
        ...
        **kwargs,
        ...
        data : dict[str, jnp.ndarray]
            Data evaluated on grid. Must include names in ``Bounce.required_names()``.
        quad : (jnp.ndarray, jnp.ndarray)
            Quadrature points xₖ and weights wₖ for the approximate evaluation of an
            integral ∫₋₁¹ g(x) dx = ∑ₖ wₖ g(xₖ). Default is 32 points.
        automorphism : (Callable, Callable) or None
        ...
        assert self.dB_dz.shape[0] == degree
        assert self.B.shape[-1] == self.dB_dz.shape[-1] == grid.num_zeta - 1
    ...
    def reshape_data(grid, data, names):
        ...
        if isinstance(names, str):
            names = [names]
        f = [
            grid.meshgrid_reshape(data[name], "raz").reshape(-1, grid.num_zeta)
            for name in names
        ]
        return f
    ...
    def check_bounce_points(self, bp1, bp2, pitch, plot=True, **kwargs):
        ...
        P, S, _ = bp1.shape
        for s in range(S):
            B = PPoly(self.B[:, s], self.zeta)
            for p in range(P):
                B_m = B((bp1[p, s] + bp2[p, s]) / 2)
                err_3 = jnp.any(B_m > 1 / pitch[p, s] + eps)
                ...
                if plot:
                    self._plot1d(
                        ...
                        title=kwargs.pop(
                            "title",
                            r"Intersects $\zeta$ for $\vertB(\zeta)\vert = 1/\lambda$",
                        ),
                        hlabel=kwargs.pop("hlabel", r"$\zeta$"),
                        vlabel=kwargs.pop("vlabel", r"$\vertB\vert(\zeta)$"),
                        **kwargs,
                    )
                print("      bp1    |    bp2")
                ...
                assert not err_3, (
                    f"Detected |B| = {B_m[mask[p, s]]} > {1 / pitch[p, s] + eps} = 1/λ "
                    "in well. Use more knots.\n"
                )
            if plot:
                self._plot1d(
                    ppoly=B,
    ...
    def integrate(
        ...
        num_well : int or None
            If not specified, then all bounce integrals are returned in an array whose
            last axis has size ``(N-1)*degree``. If there were less than that many
            wells detected along a field line, then the last axis of the returned array,
            which enumerates bounce integrals for a particular field line and pitch,
            is padded with zero.
        ...
        result = _bounce_quadrature(
            ...
            integrand=integrand,
            f=f,
            B_sup_z=self._data["B^zeta"],
            B_sup_z_ra=self._data["B^zeta_z"],
            B=self._data["|B|"],
            B_z_ra=self._data["|B|"],
            pitch=pitch,
            knots=self.zeta,
        ...
        assert result.shape[-1] == setdefault(num_well, (self.zeta.size - 1) * 3)
        return result
    ...
    def _plot1d(
        ...
        """Plot the function ``f`` defined by the piecewise polynomial.

        Parameters
        ----------
        ppoly : PPoly
            Spline of f over ``self.zeta``.
        z1 : jnp.ndarray
            Shape (k.shape[0], W).
        ...
        z = jnp.linspace(start=self.zeta[0], stop=self.zeta[-1], num=num)
        add2legend(legend, ax.plot(z, ppoly(z), label=vlabel))
        plot_intersect(ax, legend, z1, z2, k, k_transparency)
