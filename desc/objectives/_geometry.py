...
class GXGeometry(_Objective):
    """Target a particular value for one of the several flux tube coefficients.
    ...
    _coordinates = "r"
    _units = "~"
    _print_value_fmt = "GX coefficient: "
    ...
    def build(self, use_jit=True, verbose=1):
        ...
        self._dim_f = self._nturns * self._nzetaperturn
        transforms = get_transforms(self._iota_keys, eq, iota_grid)
        profiles = get_profiles(self._iota_keys + [self._name], eq, iota_grid)
        self._constants = {
    ...
    def compute(self, params, constants=None):
        ...
        data = compute_fun(
            ...
            [self._name],
            params,
            transforms=get_transforms([self._name], eq, grid, jitable=True),
            profiles=constants["profiles"],
            data=data,
