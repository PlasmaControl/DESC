...
# --- helpers still at module level ---
def _compute_B_sour(args):
    return B_sour_vec(*args)

def _compute_B_cont(args):
    return B_theta_contours_vec(*args)

def _compute_B_sticks(args):
    return B_sticks_vec(*args)
...
def bn_res_vec(
    ...
    sour_args = (sdata1, sdata2, sdata3, sgrid, surface, N, d_0, coords, tdata, ss_data)
    cont_args = (surface, coords, contour_data, contour_grid, AAA)
    stick_args = (sgrid, surface, coords, stick_data)

    # run in parallel using threads
    with ThreadPoolExecutor() as executor:
        futures = [
            executor.submit(_compute_B_sour, sour_args),
            executor.submit(_compute_B_cont, cont_args),
            executor.submit(_compute_B_sticks, stick_args),
        ]
        B_sour0, B_wire_cont, B_sticks0 = [f.result() for f in futures]
