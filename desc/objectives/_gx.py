...
class GX(_Objective):
    """Calls the gyrokinetc code GX to compute the nonlinear turbulent heat flux.
    ...
    _linear = False
    _units = "Q"
    _print_value_fmt = "Total heat flux: "
    ...
    def __init__(
        ...
        alpha=0,
        psi=0.5,
        path="/global/homes/p/pkim18/gx_next/",
        path_in="/pscratch/sd/p/pkim18/DESC/GX/gx_nl",
        path_geo="/pscratch/sd/p/pkim18/DESC/GX/gxinput_wrap",
        t=0,
        bounds=None,
        normalize=False,
        ...
        self._callback_fmt = "Total heat flux: {:10.3e} " + self._units
    ...
    def build(self, use_jit=False, verbose=1):
        ...
        if self.grid is None:
            ...
            iota = fi(rho)
            zeta = np.linspace(
                (-np.pi * self.npol - self.alpha) / np.abs(iota),
                (np.pi * self.npol - self.alpha) / np.abs(iota),
                2 * self.nzgrid + 1,
            )
            theta_sfl = (
                iota / np.abs(iota) * self.alpha * np.ones(len(zeta)) + iota * zeta
            )

            # zeta_center = zeta[self.nzgrid]              # noqa: E800
    ...
    def compute_impl(self, params, constants):
        ...
        (
            ...
            cvdrift,
            cvdrift0,
        ) = self.compute_geometry(params, constants)
        self.get_gx_arrays(
            zeta,
            bmag,
            grho,
    ...
    def compute_geometry(self, params, constants):
        """Compute the geometry of the flux tube."""
        rho = np.sqrt(self.psi)
        data_eq = compute_fun(
        ...
        zeta = np.linspace(
            (-np.pi * self.npol - self.alpha) / np.abs(iotas),
            (np.pi * self.npol - self.alpha) / np.abs(iotas),
            2 * self.nzgrid + 1,
        )
        ...
        theta_sfl = (
            iotas / np.abs(iotas) * self.alpha * np.ones(len(zeta)) + iota * zeta
        )
        ...
        # th = coords[:, 1]             # noqa: E800

        if self._profiles_eq["iota"] is None:
            self.grid = Grid(coords)
            self._transforms = get_transforms(
                self._field_line_keys, obj=self.eq, grid=self.grid
            )
        ...
        # grad_alpha = np.sqrt(          # noqa: E800
        #     grad_alpha_r**2 * data["g^rr"]             # noqa: E800
        #     + grad_alpha_t**2 * data["g^tt"]             # noqa: E800
        #     + grad_alpha_z**2 * data["g^zz"]             # noqa: E800
        #     + 2 * grad_alpha_r * grad_alpha_t * data["g^rt"]             # noqa: E800
        #     + 2 * grad_alpha_r * grad_alpha_z * data["g^rz"]             # noqa: E800
        #     + 2 * grad_alpha_t * grad_alpha_z * data["g^tz"]             # noqa: E800
        # )             # noqa: E800
        # grad_alpha = data['grad(alpha)']             # noqa: E800
        # grad_alpha = np.zeros(grad_psi.shape)             # noqa: E800
        # grad_alpha[:,0] = grad_alpha_r             # noqa: E800
        # grad_alpha[:,1] = grad_alpha_t             # noqa: E800
        # grad_alpha[:,2] = grad_alpha_z             # noqa: E800
        grad_alpha = (
        ...
        #        gds2 = grad_alpha**2 * Lref**2 *self.psi             # noqa: E800
    ...
    def get_gx_arrays(
        ...
        # final_theta_grid = self.uniform_zgrid             # noqa: E800
    ...
    def write_nc(self, t):
        ...
        # geo_dim = ncfile.createDimension("dim", len(self.bmag_gx))       # noqa: E800
        # shat_dim = ncfile.createDimension("shat_dim", 1)             # noqa: E800
    ...
    def run_gx(self):
        """run_gx."""
        # stdout = "stdout.out_" + str(self.t)        # noqa: E800
        # stderr = "stderr.out_" + str(self.t)        # noqa: E800
    ...
    def compute_gx_jvp(self, values, tangents):
        """compute_gx_jvp."""
        # R_lmn, Z_lmn, L_lmn, i_l, c_l, p_l, Psi = values             # noqa: E800
        ...
        jvp = FiniteDiffDerivative.compute_jvp(
            self.compute, argnum, tangents, *values, rel_step=1e-2
        )
