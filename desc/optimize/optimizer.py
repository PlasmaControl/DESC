...
class Optimizer:
    def __init__(self, method):
        ...
        self.method = method
        self._scipy_least_squares_methods = ["scipy-trf", "scipy-lm", "scipy-dogbox"]
        self._scipy_minimize_methods = [
            "scipy-bfgs",
            "scipy-dogleg",
            "scipy-trust-exact",
            "scipy-trust-ncg",
        ...
        self._all_methods = (
            ...
            + self._scipy_minimize_methods
            + self._desc_scalar_methods
        )

        if self.method not in self._all_methods:
            raise NotImplementedError(
                colored(
                    "method must be one of {}".format(".".join([self._all_methods])),
                    "red",
                )
            )
