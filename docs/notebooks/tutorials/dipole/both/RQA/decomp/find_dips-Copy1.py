...
def K_decomp(p_M, p_N, d_data, sgrid, surface, dt, dz, N, d_0):
    theta = jnp.linspace(2 * jnp.pi * (1 / (p_M * 2 + 1)) * 0.5,
                        2 * jnp.pi * (1 - 0.5 / (p_M * 2 + 1)),
                        p_M * 2 + 1)
    zeta = jnp.linspace(2 * jnp.pi / surface.NFP * (1 / (p_N * 2 + 1)) * 0.5,
                       2 * jnp.pi / surface.NFP * (1 - 0.5 / (p_N * 2 + 1)),
                       p_N * 2 + 1)
    ...
    assert (p_M * 2 + 1) * (p_N * 2 + 1) == r, "Dipole locations mismatch"

    def solve_single(i):
        w_i = d_data['w'][i]
        lam = d_data['lambda_iso'][i]
        e_u, e_v = d_data['K.e_u'][i], d_data['K.e_v'][i]

        omega_pol1 = omega_pair(d_data,
                                du_data["u_iso"][i], du_data["v_iso"][i],
                                dd_data["u_iso"][i], dd_data["v_iso"][i],
                                N, d_0)

        omega_tor1 = omega_pair(d_data,
                                dr_data["u_iso"][i], dr_data["v_iso"][i],
                                dl_data["u_iso"][i], dl_data["v_iso"][i],
                                N, d_0)
