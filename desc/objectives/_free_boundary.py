...
from desc.utils import PRINT_WIDTH, Timer, dot, errorif, parse_argname_change, warnif
...
class BConsistencyError(_Objective):
    ...
    def compute(self, eq_params, constants=None):
        ...
        B_nxB = Bplasma + eval_data["B"] / 2

        # old code g = eval_data["|e_theta x e_zeta|"]
        # old code B_err = (g * (B_nxB).T).T.flatten(order="F")
...
class BConsistencyLaplaceError(_Objective):
    """Target B_{nxB}=B_{eq} on target surfaces.

    Uses virtual casing to find K on boundary, and
    use biot-savart on that K to find the magnetic field
    from the Laplacian field at that point.
    ...
    def build(self, use_jit=True, verbose=1):
        ...
        errorif(
            src_grid.sym,
            ValueError,
            "Source grids for Laplace integral must be non-symmetric",
        )
        self._eq_data_keys = [
            "K_vc",
            "B",
        ...
        self._compute_B_from_surf_curr = _compute_A_or_B_from_CurrentPotentialField
        ...
        # the 3 components of B
        self._dim_f = (
            3 * eval_grid.num_nodes if not self._use_Bn else eval_grid.num_nodes
        )
    ...
    def compute(self, eq_params, constants=None):
        ...
        data_surf = {}
        data_surf["K"] = src_data["K_vc"]
        data_surf["|e_theta x e_zeta|"] = src_data["|e_theta x e_zeta|"]
        data_surf["x"] = jnp.vstack([src_data["R"], src_data["phi"], src_data["Z"]]).T

        B_from_K_vc = self._compute_B_from_surf_curr(
            self.things[0].surface,
            jnp.vstack([eval_data["R"], eval_data["phi"], eval_data["Z"]]).T,
            self._src_grid,
            data=data_surf,
        )
        if not self._use_Bn:
            B_err = (B_from_K_vc + eval_data["B"]).flatten(order="F")
        else:
            B_err = dot(B_from_K_vc, eval_data["n_rho"], axis=1)
