...
scan_configs = [
    ("$N = M$",                                      lambda res, ar: res),
    ("$N = 2M$",                                     lambda res, ar: 2 * res),
    ("$N = \\lfloor ar \\cdot M \\rfloor$",          lambda res, ar: np.int64(res * ar)),
]
n_scans = len(scan_configs)
n_res = len(resolutions)

# Root mean square errors: shape (n_scans, n_res)
B_t_errs_all = np.zeros((n_scans, n_res), dtype=float)
B_z_errs_all = np.zeros((n_scans, n_res), dtype=float)
phi_errs_all  = np.zeros((n_scans, n_res), dtype=float)
path_exists_all = np.ones((n_scans, n_res), dtype=bool)
total_pts_all = np.zeros((n_scans, n_res), dtype=int)

just_plot = True

for s, (scan_label, N_func) in enumerate(scan_configs):
    # Row-slice aliases so the inner loop body can use B_t_errs[i] etc. unchanged
    B_t_errs = B_t_errs_all[s]
    B_z_errs = B_z_errs_all[s]
    phi_errs  = phi_errs_all[s]
    path_exists = path_exists_all[s]
    ...
    for i, res in enumerate(resolutions):
        ...
        total_pts_all[s, i] = n_theta * n_zeta
...
ax[1].set_xlabel("Total grid points ($n_\\theta \\times n_\\zeta$)", fontsize=12)
ax[1].set_ylabel("RMS error in $\\mathbf{B} \\cdot \\mathbf{e}_\\zeta$", fontsize=12)
ax[2].set_xlabel("Total grid points ($n_\\theta \\times n_\\zeta$)", fontsize=12)
...
fig.tight_layout()
fig.savefig(plot_path + f"error_plot_scan_comparison.png", dpi=150)
