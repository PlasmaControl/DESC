    ...
    for ii in range(arr_len):
        ...
        if ii == 0:
            ...
            if vmec_path is not None:
                equil_fam[ii] = Equilibrium.from_configuration(
                    VMECIO.load(vmec_path, L=L[ii], M=M[ii], N=N[ii], index=zern_mode)
                )
            equil = equil_fam[ii]
        ...
        # continuing from previous solution
            ...
            equil.change_resolution(
                L=L[ii], M=M[ii], N=N[ii], M_grid=M_grid[ii], N_grid=N_grid[ii]
            )
            equil.precompute_transforms(verbose)
            ...
            if np.any(deltas):
                raise NotImplementedError(
                    "perturbations don't work with new stuff, yell at someone about that."
                )
        ...
        equil.objective = errr_mode
        equil.optimizer = optim_method
