...
# ── Finite-n eigenfunction plotting (FinitenStability.compute_and_plot) ─────
# Mirrors scripts/stability_helpers.py's `add_bc`/`save_eigenfunction_plots`,
# used by scripts/external_mode.py, adapted to read straight out of
# FinitenStability.compute_data's "... rayleigh" output instead of a
# hand-rolled dense eigsh + host reshaping pipeline.
_FINITEN_RHO_COLORS = ["tab:blue", "tab:orange", "tab:green", "tab:red"]
_FINITEN_XI_LABELS = [r"$\xi^\rho$", r"$\xi^\theta$", r"$\xi^\zeta$"]
_FINITEN_COMP_COLORS = ["steelblue", "darkorange", "seagreen"]


def _finiten_add_periodic_bc(xi_full, n_rho, n_theta, n_zeta):
    """Reshape a flattened, component-major xi vector and wrap it periodically.

    ``xi_full`` is ``"finite-n xi rayleigh"``'s (and the dense path's
    ``"finite-n xi"``'s) layout: length ``3*n_rho*n_theta*n_zeta``,
    ``[xi^rho, xi^theta, xi^zeta]`` stacked in that order, each itself a
    flattened ``(n_rho, n_theta, n_zeta)`` array (rho slowest, zeta
    fastest -- AGNI order). Returns the three components reshaped to
    ``(n_rho, n_theta, n_zeta)`` and wrapped with one extra periodic point
    in theta and zeta each -- shape ``(n_rho, n_theta + 1, n_zeta + 1)`` --
    so a plot over the full ``[0, 2*pi]`` range closes continuously. Same
    operation as ``scripts/stability_helpers.py``'s ``add_bc``.
    """
    n_total = n_rho * n_theta * n_zeta
    xi_rho0 = np.reshape(xi_full[:n_total], (n_rho, n_theta, n_zeta))
    xi_theta0 = np.reshape(xi_full[n_total : 2 * n_total], (n_rho, n_theta, n_zeta))
    xi_zeta0 = np.reshape(xi_full[2 * n_total :], (n_rho, n_theta, n_zeta))
    ...
    return _wrap(xi_rho0), _wrap(xi_theta0), _wrap(xi_zeta0)


def _finiten_save_eigenfunction_plots(
    plot_path,
    xi_rho,
    xi_theta,
    xi_zeta,
    ...
    """Save (and/or show) the 4 finite-n eigenfunction diagnostic plots.

    Same 4 plots as ``scripts/stability_helpers.py``'s
    ``save_eigenfunction_plots``: xi^rho/xi^theta/xi^zeta vs rho at
    theta=0 and theta~pi/2, and vs theta at a few rho slices (plus a
    dedicated boundary-check panel at the outermost rho) -- all evaluated
    at the zeta=0 slice, matching that reference.
    """
    ...
    label_fsize, title_fsize, suptitle_fsize = 18, 20, 24

    def _finish(fig, fname_suffix):
        fig.tight_layout()
        if plot_path is not None:
            fig.savefig(os.path.join(plot_path, f"{fname_suffix}_{tag}.png"), dpi=150)
        if show:
            plt.show()
        plt.close(fig)
    ...
    def _plot_vs_rho(i_theta, theta_label, fname_suffix):
        ...
        for xi, label, color in zip(
            [xi_rho, xi_theta, xi_zeta], _FINITEN_XI_LABELS, _FINITEN_COMP_COLORS
        ):
        ...
        ax.set_title(title + f",  {theta_label}", fontsize=title_fsize)
        ax.tick_params(labelsize=12)
        ax.legend(fontsize=13)
        ax.axhline(0, color="gray", lw=0.7, ls="--")
        _finish(fig, fname_suffix)

    def _plot_vs_theta(rho_indices, suptitle_extra, fname_suffix):
        fig, axes = plt.subplots(1, 3, figsize=(16, 5), sharey=False)
        for ax, xi, label in zip(axes, [xi_rho, xi_theta, xi_zeta], _FINITEN_XI_LABELS):
            for i_rho, color in zip(rho_indices, _FINITEN_RHO_COLORS):
                ax.plot(
                    theta,
                    xi[i_rho, :n_theta_loc, 0],
                    color=color,
                    lw=2,
        ...
        fig.suptitle(title + suptitle_extra, fontsize=suptitle_fsize)
        _finish(fig, fname_suffix)
...
class FinitenStability(_Objective):
    ...
    def compute_and_plot(
        ...
        """Compute the finite-n stability eigenmode, and plot eigenfunctions.

        Evaluates ``"finite-n lambda3 rayleigh"`` exactly like ``compute``/
        ``compute_data``, and returns the FULL data dict rather than just
        the eigenvalue: the compute chain behind it (``_AGNI3_rayleigh`` in
        ``desc/compute/_stability.py``) already stashes the full
        eigenfunction/displacement/perturbation fields as a side effect
        (``_agni3_store_rayleigh_mode_data``), under the ``"... rayleigh"``
        keys -- this just surfaces all of that, at no extra eigensolve
        cost. Also saves the same eigenfunction diagnostic plots
        ``scripts/stability_helpers.py``'s ``save_eigenfunction_plots``
        produces for ``scripts/external_mode.py``: xi^rho/xi^theta/xi^zeta
        vs rho at fixed theta, and vs theta at a few rho slices (all at the
        zeta=0 slice, matching that reference).
        ...
        params : dict
            ...
            ``Equilibrium.params_dict``.
        plot_path : str
            Directory the plots are saved to (created if it doesn't exist).
        constants : dict
            Dictionary of constant data, e.g. transforms, profiles etc.
            Defaults to ``self.constants``.
        tag : str, optional
            Suffix appended to each plot's filename, e.g. an identifying
            run tag.
        title : str, optional
            Plot title. Defaults to a summary built from ``n_mode_axisym``
            (when ``axisym``) and the solved eigenvalue.
        rho_iota1 : float, optional
            rho location of the iota=1 surface, if any -- drawn as a
            vertical reference line on the "vs rho" plots when given.
        show : bool, optional
            Whether to also display the plots interactively via
            ``plt.show()`` (default False).

        Returns
        -------
        data : dict
            Full compute output: ``"finite-n lambda3 rayleigh"`` and its
            eigenfunction/xi/deltaB/deltaV siblings (see
            ``_agni3_store_rayleigh_mode_data``), in both flattened/reduced
            and physical ``(n_rho, n_theta, n_zeta)`` forms.

        """
        constants = self._constants if constants is None else constants
        data = self.compute_data(params, constants=constants, solve=False)

        n_rho = self._grid.num_rho
        n_theta = self._grid.num_theta
        n_zeta = self._grid.num_zeta
        rho = np.asarray(self._fine_rho1d)
        theta = np.asarray(self._grid.unique_theta)

        lam = float(
            np.real(np.asarray(data["finite-n lambda3 rayleigh"]).reshape(-1)[0])
        )
        if title is None:
            title = (
                r"Finite-$n$ Rayleigh eigenmode"
                + (f" (n={self._n_mode_axisym})" if self._axisym else "")
                + rf",  $\lambda_R = {lam:.3e}$"
            )

        # "finite-n xi rayleigh" carries the same arbitrary overall complex
        # phase `_agni3_store_rayleigh_mode_data` rotates away before taking
        # deltaB/deltaV -- match that convention here (xi_full[:n_total] is
        # the xi^rho component, same as its `xi_ref`), so the plotted mode
        # isn't an arbitrary near-zero rotation of the real eigenfunction.
        xi_full = np.asarray(data["finite-n xi rayleigh"])
        if np.iscomplexobj(xi_full):
            xi_ref = xi_full[: n_rho * n_theta * n_zeta]
            rot = np.exp(1j * np.arctan2(np.mean(xi_ref.real), np.mean(xi_ref.imag)))
            xi_full = (xi_full * rot).imag

        xi_rho, xi_theta, xi_zeta = _finiten_add_periodic_bc(
            xi_full, n_rho, n_theta, n_zeta
        )

        os.makedirs(plot_path, exist_ok=True)
        _finiten_save_eigenfunction_plots(
            plot_path,
            xi_rho,
            xi_theta,
            xi_zeta,
