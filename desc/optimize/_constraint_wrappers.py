...
class FiniteDifferenceFreeBoundary(ObjectiveFunction):
    ...
    def __init__(
        ...
        perturb_options=None,
        linear_constraints_coil_profiles_etc=None,
        findif_options={},
...
class ProximalProjectionFreeBoundary(ObjectiveFunction):
    ...
    At each iteration, after a step is taken to reduce the objective, the equilibrium
    is re-solved to bring it back into free boundary force balance. This is analogous
    to a proximal method where each iterate is projected back onto the feasible set.
    ...
    def _update_equilibrium(self, x, store=False):
        """Update the internal equilibrium with new profile/Psi.
        ...
        if xopt.size > 0 and xeq.size > 0:
            ...
            # We pass in the LinearConstraintProjection object to skip some redundant
            # computations in the perturb and solve methods
            # TODO: is this perturb necessary?
            self._eq = self._eq.perturb(
                objective=self._eq_solve_objective,
                constraints=None,
                deltas=deltas,
    ...
    def _jvp(self, v, xf, xg, constants, op):
        ...
        dfdc = _proximal_jvp_f_pure_fb(
            self._constraint,
            xf,
            constants[1][
                0
            ],  # extra index bc constraints[1] is a list of [bdry erro const, fbal]
            vs[self._eq_idx],
            self._unfixed_idx,
            self._Z,
            self._D,
        ...
        if self._objective._deriv_mode in ["batched"]:
            ...
            out = _proximal_jvp_blocked_pure_fb(self._objective, vgs, xgs, op)
        return -out
...
@functools.partial(jit, static_argnames=["op"])
def _proximal_jvp_f_pure_fb(constraint, xf, constants, dc, unfixed_idx, Z, D, dxdc, op):
    Fx = getattr(constraint._objective, "jac_" + op)(xf, constants)
...
# @functools.partial(jit, static_argnames=["op"])
def _proximal_jvp_blocked_pure_fb(objective, vgs, xgs, op):
    out = []
    for k, (obj, const) in enumerate(zip(objective.objectives, objective.constants)):
        thing_idx = objective._things_per_objective_idx[k]
        xi = [xgs[i] for i in thing_idx]
...
class ProximalProjectionFB2(ObjectiveFunction):
    """Remove equilibrium constraint by projecting onto constraint at each step.
    ...
    def __init__(
        ...
        solve_options = {} if solve_options is None else solve_options
        perturb_options = {} if perturb_options is None else perturb_options
        fb_options = {} if fb_options is None else fb_options
    ...
    def build(self, use_jit=None, verbose=1):  # noqa: C901
        ...
        eq = self._eq
        timer = Timer()
        timer.start("Proximal fb projection build")
