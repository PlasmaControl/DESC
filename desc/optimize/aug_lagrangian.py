...
def fmin_lag_stel(
    ...
    print("The initial obj func is " + str(f))
    cv = L.compute_constraints(x)
    print("The constraint term is " + str(-jnp.dot(lmbda,cv)+mu*jnp.dot(cv,cv))) 

    while iteration < maxiter:
        xk = fmintr(L.compute,x,gradL,args=(lmbda,mu,),gtol=gtolk,maxiter=10,verbose=2)
