...
class TestBasis:
    ...
    def test_chebyshev(self):
        ...
        # Chebyshev values
        correct_values = np.array([np.ones_like(r), 2 * r - 1, 8 * r**2 - 8 * r + 1]).T
        values = chebyshev(r[:, np.newaxis], l, dr=0)
        np.testing.assert_allclose(values, correct_values, atol=1e-8)

        # 1st-order derivatives
        correct_derivs1 = np.array([np.zeros_like(r), np.full_like(r, 2), 16 * r - 8]).T
        derivs1 = chebyshev(r[:, np.newaxis], l, dr=1)
        np.testing.assert_allclose(derivs1, correct_derivs1, atol=1e-8)

        # 2nd-order derivatives
        correct_derivs2 = np.array(
            [np.zeros_like(r), np.zeros_like(r), np.full_like(r, 16)]
        ).T
        derivs2 = chebyshev(r[:, np.newaxis], l, dr=2)
        np.testing.assert_allclose(derivs2, correct_derivs2, atol=1e-8)
