...
ax.plot(results_iota0, results_lambda_min, "o-", color="steelblue", lw=2, ms=7)
ax.set_xlabel(r"$\iota$ on axis ($\iota_0$)", fontsize=14)
ax.set_ylabel(r"$\lambda_{\min}$", fontsize=14)
ax.set_title(
    r"Stability eigenvalue vs $\iota_0$" + "\n"
    r"($\iota(\rho) = \iota_0 - 0.05\,\rho^2$,  $\mathrm{d}^2\iota/\mathrm{d}\rho^2 = -0.1$)",
    fontsize=12,
)
