...
def compute_coordinate_derivatives(cR, cZ, zernt, zeta_ratio=1.0, mode='equil'):
    ...
    # extra terms needed for axis & QS
...
def compute_covariant_basis(coord_der, zernt, mode='equil'):
...
def compute_contravariant_basis(coord_der, cov_basis, jacobian, zernt):
...
def compute_jacobian(coord_der, cov_basis, zernt, mode='equil'):
...
def compute_B_field(cov_basis, jacobian, cI, Psi_total, zernt, mode='equil'):
    ...
    # extra terms needed for axis or QS
    if len(axn) or mode == 'qs':
        ...
        B_field['B^zeta_vz'] = - (B_field['psi_r']*jacobian['g_vz']) / (2*jnp.pi*jacobian['g']**2) \
            + (B_field['psi_r']*jacobian['g_v']*jacobian['g_z']) / \
            (jnp.pi*jacobian['g']**3)
    ...
    # axis values
    if len(axn):
        B_field['B^zeta_r'] = put(B_field['B^zeta_r'], axn, -(B_field['psi_rr']
                                                              [axn]*jacobian['g_rr'][axn]) / (4*jnp.pi*jacobian['g_r'][axn]**2))
        B_field['B^zeta_v'] = put(B_field['B^zeta_v'], axn, 0)
        B_field['B^zeta_z'] = put(B_field['B^zeta_z'], axn, -(B_field['psi_rr']
...
def compute_J_field(coord_der, cov_basis, jacobian, B_field, cI, Psi_total, zernt):
    ...
    Args:
        ...
        coord_der (dict): dictionary of ndarray containing of coordinate 
            derivatives evaluated at node locations, such as computed by ``compute_coordinate_derivatives``.
        B_field (dict): dictionary of ndarray containing magnetic field and derivatives, 
            such as computed by ``compute_B_field``.
        cI (array-like): coefficients to pass to rotational transform function.
        Psi_total (float): total toroidal flux within LCFS.
        zernt (ZernikeTransform): object with transform method to go from spectral to physical space with derivatives
    ...
    # axis values
    if len(axn):
        J_field['J^rho'] = put(J_field['J^rho'], axn, (Bsub_zeta_rv[axn] -
                                                       Bsub_theta_rz[axn]) / (jacobian['g_r'][axn]))
...
def compute_B_magnitude(cov_basis, B_field, cI, zernt):
    ...
    B_mag['|B|'] = jnp.abs(B_field['B^\zeta'])*jnp.sqrt(iota**2*dot(cov_basis['e_theta'], cov_basis['e_theta']) +
...
def compute_F_magnitude(coord_der, cov_basis, con_basis, jacobian, B_field, J_field, cP, cI, Psi_total, zernt):
    ...
    Args:
        ...
        B_field (dict): dictionary of ndarray containing magnetic field and derivatives, 
            such as computed by ``compute_B_field``.
        J_field (dict): dictionary of ndarray containing current and derivatives, 
            such as computed by ``compute_J_field``.
    ...
    Return:
        F_mag (ndarray, shape(N_nodes,)): force error magnitudes at each node.
    ...
    if len(axn):
        ...
        F_theta = put(
            F_theta, axn, J_field['J^rho'][axn]*B_field['B^zeta'][axn])
        F_zeta = put(F_zeta, axn, -J_field['J^rho']
                     [axn]*B_field['B^theta'][axn])
