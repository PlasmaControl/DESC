...
def fmin_scalar(fun, x0, grad, 
    ...
    ftol : float or None, optional
        ...
        is 1e-6. The optimization process is stopped when ``dF < ftol * F``,
        and there was an adequate agreement between a local quadratic model and
        the true model in the last step. If None, the termination by this
        condition is disabled.
    xtol : float or None, optional
        Tolerance for termination by the change of the independent variables.
        Default is 1e-6. Optimization is stopped when
    ...
    rgtol : float or None, optional
        Relative tolerance for termination by the norm of the change in thegradient. 
        Default is 1e-6. Optimizer teriminates when
   ...
   verbose : {0, 1, 2}, optional
    ...
    hess_recompute_freq = options.pop('hessian_recompute_interval',1 if callable(hess) else 0)
    ...
    while True:
        ...
        if ga_accept_threshold > 0:
            ...
            ga_ratio = np.linalg.norm(scale*ga_step_h, ord=xnorm_ord)/np.linalg.norm(scale*step_h, ord=xnorm_ord)
        ...
        predicted_reduction = f - evaluate_quadratic_form(step_h, f, g, hess, scale=scale, sqr=False)
