...
def K_sour(p_M, p_N,
    ...
    return ( ( sdata["lambda_iso"] ** (-1) )*( - omega_total_imag * sdata["e_v"].T #- cross(sdata["n_rho"],sdata["e_u"]).T 
                                                + omega_total_real * sdata["e_u"].T#- cross(sdata["n_rho"],sdata["e_v"]).T
                                             )
            ).T
...
def B_theta_contours(p_M, p_N,
    ...
    zeta_coarse = jnp.linspace(2 * jnp.pi / sgrid.NFP * (1 / (p_N * 2)) * 1/2,
                               2 * jnp.pi / sgrid.NFP * (1 - 1 / (p_N * 2) * 1/2),
                               p_N * 2,)
...
def bn_res(p_M, p_N, sdata, sgrid, surface, #w_surface,
    ...
    B0 = B_sour(p_M, p_N, sdata, sgrid, surface,
                y, N, d_0, eq, Bgrid)
...
def B_contour(grid_cont,
              K_cont, 
              surface, eq,egrid):
    
    B_cont = _compute_magnetic_field_from_Current(grid_cont, 
                                                  K_cont,
                                                  surface, eq, egrid, basis="rpz")
...
def stick(p2, # second point of the stick
    ...
    def nfp_loop(j, f):
        ...
        phi = ( p2_[2] + j * 2 * jnp.pi / surface_grid.NFP ) % ( 2 * jnp.pi )
        #(surface_grid.nodes[:, 2] + j * 2 * jnp.pi / surface_grid.NFP ) % ( 2 * jnp.pi )

        p2s = jnp.vstack((p2_[0], phi, p2_[2])).T
        p2s = rpz2xyz(p2s)
        a_s = p2s - p1
        
        b_s = p1 - plasma_points
