...
def _compute_magnetic_field_from_Current_vec(Kgrid, K_at_grid, surface, data, coords,
    ...
    """MPI-aware Biot-Savart with 2D splitting across M (sources) and N (JdV axis)."""
    if mpi_comm is None:
    ...
    N = K_at_grid.shape[0]   # expansion index
    M = K_at_grid.shape[1]   # source grid size
    ...
    # split along M
    start_src, end_src = _split_indices(M, i_M, P_M)
    ...
    local_K = local_K[start_N:end_N, :, :]

    # handle empty local_K or local_rs
    if local_rs.shape[0] == 0 or local_K.shape[0] == 0:
        local_coords = coords_xyz if not split_targets else coords_xyz[_split_indices(coords_xyz.shape[0], rank, size)[0]:_split_indices(coords_xyz.shape[0], rank, size)[1]]
        B_local_np = np.zeros((end_N-start_N, 3, local_coords.shape[0]), dtype=np.float64)
    ...
    B_local = fori_loop(0, Kgrid.NFP, nfp_loop_local,
                        jnp.zeros((end_N - start_N, 3, local_coords.shape[0])))
...
def _compute_magnetic_field_from_Current_Contour_vec(Kgrid, K_at_grid, surface, data, coords,
    ...
    if local_rs.shape[0]==0 or local_K.shape[0]==0:
        local_coords = coords_xyz if not split_targets else coords_xyz[_split_indices(coords_xyz.shape[0], rank, size)[0]:_split_indices(coords_xyz.shape[0], rank, size)[1]]
        B_local_np = np.zeros((N,3,local_coords.shape[0]), dtype=np.float64)
        if mpi_comm is not None:
            mpi_comm.Barrier()
        return B_local_np
...
def B_sticks_vec(sgrid, surface, coords, ss_data, mpi_comm=None, split_targets=True):
    ...
    if p2_local.shape[0]==0:
        b_local_np = np.zeros((0, N_coords, 3), dtype=np.float64)
        if mpi_comm is not None:
            mpi_comm.Barrier()
        return b_local_np
...
# ---------------- MPI-aware bn_res_vec_2d ----------------
def bn_res_vec_mpi_2d(sdata1,sdata2,sdata3,sgrid,surface,N,d_0,coords,tdata,
    ...
    B_sticks0 = B_sticks_vec(sgrid, surface, coords, stick_data,
                             mpi_comm=mpi_comm, split_targets=split_targets)

    #B_sour0 = jnp.transpose(B_sour0, (0, 2, 1))        # (sources, targets, 3)
    #B_wire_cont = jnp.transpose(B_wire_cont, (0, 2, 1)) # same
    B_sticks0 = jnp.transpose(B_sticks0, (0, 2, 1))    # same

    # transpose to shape (targets, components, sources)
