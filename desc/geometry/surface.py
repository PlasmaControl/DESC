...
def convert_spectral_to_FE(
    ...
    t1 = time.time()
    FE_basis_pre_evaluated = Rprime_basis.evaluate(nodes=quadpoints)
    t2 = time.time()
    ...
    RR = R_sum_pre_evaluated[:, np.newaxis] * FE_basis_pre_evaluated
    LL = L_sum_pre_evaluated[:, np.newaxis] * FE_basis_pre_evaluated
