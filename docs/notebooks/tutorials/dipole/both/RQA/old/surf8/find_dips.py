...
def K_dips(p_M, p_N, sdata, sgrid, surface, y, dt, dz, N, d_0):
    ...
    if rank == 0:
        print(f"Number of JAX devices: {jax.device_count()}, Devices: {jax.devices()}")
    ...
    # Split indices across all tasks (devices)
    num_devices = jax.device_count()  # E.g., 16 for 2 nodes with 8 tasks each
    indices = jnp.arange(r)
    indices_per_device = (r + num_devices - 1) // num_devices
    start = rank * indices_per_device
    end = min((rank + 1) * indices_per_device, r)
    chunk = indices[start:end]
    padded_chunk = jnp.pad(chunk, (0, indices_per_device - len(chunk)), 
                          mode='constant', constant_values=-1)
    if rank == 0:
        print(f"Task {rank}: Processing indices {chunk.tolist()}")
    ...
    # Run scan on this task's chunk
    omega_real, omega_imag = compute_chunk(padded_chunk)

    # Aggregate results across tasks using all-reduce
    comm = MPI.COMM_WORLD
    omega_real = comm.allreduce(omega_real, op=MPI.SUM)
    omega_imag = comm.allreduce(omega_imag, op=MPI.SUM)

    return ((sdata["lambda_iso"] ** (-1)) * (
            omega_imag * -cross(sdata["n_rho"], sdata["e_u"]).T 
            + omega_real * -cross(sdata["n_rho"], sdata["e_v"]).T
    )).T
