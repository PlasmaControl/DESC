...
class CoilSetMinDistance(_Objective):
    ...
    def compute(self, params, constants=None):
        ...
        if self._n_neighbors is not None and self._n_neighbors < n_coils - 1:
            ...
            centroid_dists = safenorm(
                centroids[:, None] - centroids[None, :], axis=-1
            )
            centroid_dists = centroid_dists.at[jnp.diag_indices(n_coils)].set(
                jnp.inf
            )
            ...
            def body(k):
                ...
                neighbor_idx = jax.lax.stop_gradient(
                    jnp.argsort(centroid_dists[k])[:K]
                )
                coil_pts = pts[k]
            ...
            def body(k):
                ...
                dist = safenorm(
                    coil_pts[None, :, None] - other_pts[:, None], axis=-1
                )
                if self._use_softmin:
                    return softmin(dist, self._softmin_alpha)
                return jnp.min(dist)
