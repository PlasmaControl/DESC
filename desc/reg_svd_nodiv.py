...
def y_soln(x,V_mn,
    ...
    error_div = lambda x: divergence_sigma(x, V_mn,
                                           data_center, grid_center,
                                           delta_hor_edge, delta_ver_edge,
                                           delta_hor_edge_l, delta_hor_edge_r,
                                           delta_ver_edge_d, delta_ver_edge_u,
    ...
    eps = 1e-3
    W = jnp.diag(S / jnp.sqrt(S**2 + eps**2))
    ...
    # Quadratic system
    lhs = Ap.T @ Ap + mu_y * jnp.eye(Z.shape[1])
    rhs = Ap.T @ B_target.flatten() + (mu_y / 2) * (Z.T @ jnp.ones(Z.shape[0]))
    
    # Solve
    x_soln = jnp.linalg.solve(lhs, rhs)
    soln = Z @ x_soln
