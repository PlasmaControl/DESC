...
# Modified K_dips for single-process CPU execution with SLURM task distribution
def K_dips(p_M, p_N, sdata, sgrid, surface, y, dt, dz, N, d_0, rank, size, output_dir="output"):
    ...
    # Split indices across SLURM tasks
    ...
    with open(partial_file, 'wb') as f:
        pickle.dump({'omega_real': np.array(omega_real), 'omega_imag': np.array(omega_imag)}, f)

    # Barrier to ensure all tasks write results
    # (Since no MPI, we rely on SLURM and file system consistency)
    if rank == 0:
        # Aggregate results
        omega_real_all = np.zeros(output_shape)
        omega_imag_all = np.zeros(output_shape)
        for i in range(size):
            with open(os.path.join(output_dir, f"partial_{i}.pkl"), 'rb') as f:
                partial = pickle.load(f)
                omega_real_all += partial['omega_real']
                omega_imag_all += partial['omega_imag']
    ...
    return result #None  # Non-rank-0 tasks return None
