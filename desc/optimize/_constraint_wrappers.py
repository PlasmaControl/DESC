...
class ProximalProjectionFreeBoundary(ObjectiveFunction):
    """Remove free boundary constraint by projecting onto constraint at each step.

    Combines objective and free boundary equilibrium constraint
    into a single objective to then pass to an unconstrained optimizer.

    The derivatives of the objective are obtained by finding the projected
    derivative of the
    ...
    def _set_eq_state_vector(self):
        ...
        self._dxdc = np.hstack(dxdc) # this is for full x to the free-bdry prox DOFs
        # which are Psi, I, iota, p, and any other non eq thing
        # we also will need a self._dxdz to go from the x 
    ...
    def build(self, use_jit=None, verbose=1):  # noqa: C901
        ...
        eq = self._eq
        timer = Timer()
        timer.start("Proximal Free Boundary projection build")
        ...
        self._built = True
        timer.stop("Proximal Free Boundary projection build")
        if verbose > 1:
            timer.disp("Proximal Free Boundary projection build")
...
# this will need to be dh/dc_fb - dh/dz @ (dg/dz)^-1 @ dg/dc_fb
# so will need to be c_fb (tangents in c_fb dirn) and then 
# the z tangents I suppose, then dgdc_fb
# tangents in c_fb dirn would come from the lin con proj on the c_fb 
# stuff I think
# z tangents I guess would be d/dx x (dc/dx)
    def _get_tangent(self, v, xf,xg, constants, op):
        ...
        #     dc=vs[self._eq_idx],
        #     eq_feasible_tangents=self._eq_solve_objective._feasible_tangents,
        #     dxdc=self._dxdc,
        #     op=op,
        # )

        # this I think is actually dgdc of ProximalProjection,
        #  we need to then take only the part corresponding to Rb Zb
        # unless the linear constraint proj already covers that since
        # it will have profiles etc fixed...
        eq_params_for_usual_prox = self._eq.unpack_params(xf)
        opt_xs_for_usuaL_prox = []
        opt_xs_for_usuaL_prox += [
                    jnp.concatenate(
                        [jnp.atleast_1d(eq_params_for_usual_prox[arg]) for arg in self._eq_fb_objective_wrapped._objective._args]
                    )
                ]
        opt_xs_for_usuaL_prox = jnp.concatenate(opt_xs_for_usuaL_prox)
        dgdz = self._eq_fb_objective_wrapped.jac_scaled_error(self._eq_fb_objective_wrapped.project(opt_xs_for_usuaL_prox))
        cutoff = jnp.finfo(dgdz.dtype).eps * max(dgdz.shape)
        uf, sf, vtf = jnp.linalg.svd(dgdz, full_matrices=False)
        # sf += sf[-1]  # add a tiny bit of regularization
        sfi = jnp.where(sf < cutoff * sf[0], 0, 1 / sf)
        # also need dgdc_fb which we get from jac_scaled_error of self._dxdc @ dc
        # but maybe also would include the other stuff like the coil DOFs...
        dgdc_fb = self._constraint_fb.jvp_scaled_error(self._dxdc@vs[self._eq_idx],
                                                                  xf,
                                                                  self._constraint_fb.constants)

        dgdz_inv_dgdc_fb = vtf.T @ (sfi * (uf.T @ dgdc_fb))

        # broadcasting against multiple things
        #TODO need to do this for dgdz_inv_dgdc_fb
        # dfdcs = [jnp.zeros(dim) for dim in self._dimc_per_thing]
        # dfdcs[self._eq_idx] = dfdc
        # # note that dfdc.size != vs[self._eq_idx].size
        # # dfdc has the size of reduced state vector of the equilibrium
        ...
        dxdcv = jnp.concatenate(
            [
                *vs[: self._eq_idx],
                self._dxdc @ vs[self._eq_idx],  # free bdry dofs (Psi, p, I,)
                *vs[self._eq_idx + 1 :],
            ]
        ) # so this term, when dh/dx @ dxdcdv, yields the dh/dc_fb term
        # where J = dh/dx is just the jacobian of our overall objective
        #TODO not sure if self._constraint_fb._dxdc needs to also account
        # for the other things, I actually assume yes like dxdcv above?
        # this is the tangent which will go from d/dx to d/dz where dz is 
        # the boundary terms only

        #TODO: how to account for other objects cleanly
        # if len(vs)[: self._eq_idx]:
        #     vs1 = jnp.concatenate(*vs[: self._eq_idx])
        #     eye1 = jnp.eye(N=vs1.shape[0], M=vs1.shape[1])
        # if *vs[self._eq_idx + 1 :]:

        # vs2 = jnp.concatenate(*vs[self._eq_idx + 1 :])
        
        dxdz = jnp.concatenate(
            [
                # jnp.eye(N=vs1.shape[0], M=vs1.shape[1]),
                jnp.vectorize(self._eq_fb_objective_wrapped.project,signature='(k)->(n)')(self._eq_fb_objective_wrapped._objective._dxdc),  # should only be the bdry modes
                # jnp.eye(N=vs2.shape[0], M=vs2.shape[1]),
            ]
        )
        tangent = dxdcv - dxdz @ dgdz_inv_dgdc_fb
        return tangent
@functools.partial(jit, static_argnames=["op"])
def _proximal_freebdry_jvp_f_pure(
    constraint, xf, constants, dc, eq_feasible_tangents, dxdc, op, dgdz
):
    # Note: This function is called by _get_tangent which is vectorized over v
    # (v is called dc in this function). So, dc is expected to be 1D array
    # of same size as full equilibrium state vector. This function returns a 1D array.

    # here we are forming (dF_fb/dz   )^-1 @ dF/dc
    # where Fxh is dF/dx and Fc is dF/dc
    # Our compute functions never include variables like Rb_lmn, Zb_lmn etc. So,
    # taking the JVP in just dc direction will give 0. To prevent this, we use dxdc
    # which is the dx/dc matrix and convert the Rb_lmn to R_lmn entries etc.
