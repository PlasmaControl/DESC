...
from desc.configuration import symmetry_matrix
...
class ObjectiveFunction(ABC):
    ...
    def __init__(self, stell_sym, errr_mode, bdry_mode, bdry_fun, RZ_transform, L_transform, bdryM, bdryN, scalar):
...
def is_nested(cR, cZ, basis, L=10, M=361, zeta=0):
    ...
    grid = LinearGrid(L=L, M=M, N=0, NFP=basis.NFP)
...
def compute_force_error_RddotZddot(cR, cZ, cP, cI, Psi_lcfs, RZ_transform, pres_transform, iota_transform, pres_ratio, zeta_ratio):
    ...
    num_nodes = len(RZ_transform.nodes.shape[0])
