...
def test_clebsch_sfl_funs(eq):
    ...
    np.testing.assert_allclose(data["B"], cross(data["grad(psi)"], data["grad(alpha)"]))
    np.testing.assert_allclose(data["B^zeta"], data["psi_r"] / data["sqrt(g)_Clebsch"])
