...
def convert_to_sfl(equil, L=None, M=None, N=None, spectral_indexing="ansi"):
    ...
    ----------
    equil : Equilibrium
        equilibrium to convert
    Lres : int
        radial resolution of conversion
    Mres : int
    ...
    -------
    R_lmn : ndarray
        toroidal R coordinate in straight field-line coordinate system
    Z_lmn : ndarray
        toroidal Z coordinate in straight field-line coordinate system
    ...
    # straight field-line grid that we want to convert to
    sfl_grid = QuadratureGrid(L=int(1.5 * L), M=3 * M + 1, N=3 * N + 1, NFP=equil.NFP)
    theta = np.zeros((sfl_grid.num_nodes,))
    ...
    # transform to nodes that correspond to the straigh field-line grid
    nodes = sfl_grid.nodes
    nodes[:, 1] = theta
    rtz_grid = Grid(nodes)
    coords = equil.compute_toroidal_coords(rtz_grid)
    R = coords["R"]
    Z = coords["Z"]

    # fit to straight field-line coordinates
    if equil.sym:
        R_sym = "cos"
        Z_sym = "sin"
    else:
