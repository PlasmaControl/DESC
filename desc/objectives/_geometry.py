...
# Import functions for Harmonic Field - objective
#from desc.equilibrium import Equilibrium # maybe not needed
#from desc.fns_simp import (find_H1, 
...
#########################################################################################################
class PlasmaVesselDistanceMod(_Objective):
    """Target the distance between the plasma and a surrounding surface.
    ...
    def __init__(
        ...
        super().__init__(
            #things=[eq, self._surface] if not surface_fixed else [eq],
            things=[self._surface] if not surface_fixed else [eq],
            target=target,
            bounds=bounds,
            weight=weight,
            normalize=normalize,
    ...
    def build(self, use_jit=True, verbose=1):
        ...
        """
        eq = self._eq#things[0]
        surface = self._surface if self._surface_fixed else self.things[0]
        # if things[1] is different than self._surface, update self._surface
        if surface != self._surface:
            self._surface = surface
        if self._surface_grid is None:
        ...
        data = compute_fun(
            "desc.equilibrium.equilibrium.Equilibrium",
            self._equil_data_keys,
            params=self._eq.params_dict,#equil_params,
    ...
    def compute(self, #equil_params, 
        ...
        d = safenorm(constants["plasma_coords"][:, None, :] - surface_coords[None, :, :], axis=-1)
        #safenorm(plasma_coords[:, None, :] - surface_coords[None, :, :], axis=-1)
