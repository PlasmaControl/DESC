...
# original resolution
bdry = np.concatenate(
    [bdry_idx, np.atleast_2d(bdryR).T, np.atleast_2d(bdryZ).T], axis=1
)
equil_nodes, equil_volumes = get_nodes_pattern(
    M0nodes, N0nodes, NFP, index=zern_mode, surfs=node_mode, sym=stell_sym, axis=False
)
bdry_nodes, _ = get_nodes_surf(M0nodes, N0nodes, NFP, surf=1.0, sym=stell_sym)
...
bdry_nodes, _ = get_nodes_surf(Mnodes, Nnodes, NFP, surf=1.0, sym=stell_sym)
...
bdry_pol, bdry_tor, bdryR, bdryZ = format_bdry(M, N, NFP, bdry, bdry_mode, bdry_mode)
x0, equil_zernike_transform, bdry_zernike_transform = expand_resolution(
    x0,
    equil_zernike_transform,
    bdry_zernike_transform,
    zern_idx_old,
    zern_idx,
    lambda_idx_old,
    lambda_idx,
...
bdryR_modes = np.where(np.logical_and(bdry_tor != 0, sign(bdry_pol) == sign(bdry_tor)))[
    0
]
bdryZ_modes = np.where(np.logical_and(bdry_tor != 0, sign(bdry_pol) != sign(bdry_tor)))[
    0
]
...
# Plunk's result
bdry_Plunk = np.concatenate(
    [
        bdry3["bdry_idx"],
        np.atleast_2d(bdry3["bdryR"]).T,
        np.atleast_2d(bdry3["bdryZ"]).T,
    ],
    axis=1,
)
...
DC = vec2mat(
    dc,
    np.concatenate([bdry_pol[bdryR_modes], bdry_pol[bdryZ_modes]]),
    np.concatenate([bdry_tor[bdryR_modes], bdry_tor[bdryZ_modes]]),
    M,
    N,
)
