...
wout_filename = "wout_precise_QA_desc.nc"

charge = ALPHA_PARTICLE_CHARGE
mass = ALPHA_PARTICLE_MASS
Ekin = 1e-5*FUSION_ALPHA_PARTICLE_ENERGY
...
bri = BoozerRadialInterpolant(
    wout_filename, order, no_K=True, comm=comm_world,
)
helicity_N = 0
...
if verbose:
    ...
    plt.figure()
    plt.xlabel("s")
    plt.ylabel(r"$\omega_{\zeta}/\omega_b$")

s_prof_list = []
omega_b_prof_list = []
omega_alpha_prof_list = []
omega_zeta_prof_list = []
for lam in lam_range:
    ...
    for s_traj, chis_traj, etas_traj, t_traj in zip(s_map, chis_map, etas_map, t_map):
        ...
        delta_zeta = np.array(zeta_traj[1::]) - np.array(zeta_traj[0:-1])
        delta_t = t_traj[1::]
        # Average over wells along one field line
        omega_alpha_list.append(np.mean(delta_alpha) / np.mean(delta_t))
        omega_zeta_list.append(np.mean(delta_zeta) / np.mean(delta_t))
    ...
    omega_zeta = np.array(omega_zeta_list)
...
if verbose:
    ...
    omega_zeta_prof=np.array(omega_zeta_prof_list, dtype=object),
    )
