...
#             )
#             iota = jnp.abs(eq.compute("iota",grid=iota_grid)["iota"][1]
...
#             # get_transforms(self._data_keys, eq, self._grid, jitable=True),
#             # profiles=get_profiles(self._data_keys, eq, self._grid),
#             data=data,
#         )
#         return data["R_eff"]
...
class EffectiveRadius(_Objective):
    ...
    def compute(self, params, constants=None):
        ...
        zeta = jnp.linspace(0,2*jnp.pi*n_tor, self._n_pol * self._knots_per_transit)
...
class ParallelConnectionLength(_Objective):
    ...
    _coordinates = ""   # Parallel connection length is a scalar parameter of each drift well 
    _units = "~"
    ...
    def compute(self, params, constants=None):
        ...
        -------
        L_par : float/ndarray
            if target_mode is max/mean, maximum or average parallel connection length along the field line
            if target_mode is all, array or effective radii along the field line            
        """
        eq = self.things[0]
