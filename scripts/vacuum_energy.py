...
def compute_external_coil_energy(coil_field, R0, a, L=50, M_quad=50, N=0, r_max_factor=20.0):
    ...
    in_plasma_factor_v = in_plasma_factor_full[valid] if N > 0 else np.ones_like(r_v, dtype=bool)
    print(in_plasma_factor_v)
...
for k, frac in enumerate(delta_h_fracs):
    ...
    W_V_true_vals[k] = compute_external_coil_energy(
        coil_k, R0, a - elongation, r_max_factor=10000, L=2**14, M_quad=2**12, N=10000,
    )
    print(
