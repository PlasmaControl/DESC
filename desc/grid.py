...
class LinearGrid(_Grid):
    ...
    def _create_nodes(  # noqa: C901
        ...
        if np.isscalar(rho) and (int(rho) == rho) and rho > 0:
            ...
            dr = _midpoint_spacing(r, use_numpy=True)
        ...
        if np.isscalar(theta) and (int(theta) == theta) and theta > 0:
            ...
            if t.size > 1:
                if not self.sym:
                    dt = _periodic_spacing(t, theta_period, use_numpy=True)[1]
        ...
        if np.isscalar(zeta) and (int(zeta) == zeta) and zeta > 0:
            ...
            z, dz = _periodic_spacing(zeta, zeta_period, sort=True, use_numpy=True)
            dz = dz * NFP
...
class ConcentricGrid(_Grid):
    ...
    def _create_nodes(self, L, M, N, NFP=1, axis=False, node_pattern="jacobi"):
        ...
        drho = _midpoint_spacing(rho, use_numpy=True)
        r = []
        t = []
        dr = []
        dt = []
...
def _periodic_spacing(x, period=2 * jnp.pi, sort=False, use_numpy=False):
...
def _midpoint_spacing(x, use_numpy=False):
