...
def _split_indices(n_items, rank, size):
    """Return start/end indices for contiguous block for MPI rank."""
    base = n_items // size
    rem = n_items % size
    if rank < rem:
        start = rank * (base + 1)
...
def B_sticks_vec(sgrid, surface, coords, ss_data, mpi_comm=None):
    ...
    Splits wires across ranks and uses Allreduce with padding.
    """
    if mpi_comm is None:
        mpi_comm = MPI.COMM_WORLD
    rank = mpi_comm.Get_rank()
    ...
    b_global = jnp.asarray(b_global_np)
...
def bn_res_vec_mpi(
    sdata1,sdata2,sdata3,sgrid,surface,N,d_0,coords,tdata,contour_data,stick_data,contour_grid,ss_data,AAA,
    mpi_comm=None
):
    if mpi_comm is None:
        mpi_comm = MPI.COMM_WORLD
