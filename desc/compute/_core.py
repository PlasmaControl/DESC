    ...
    **kwargs,
    ...
    if iota is not None:
        ...
        if check_derivs("iota", R_transform, Z_transform, L_transform):
            ...
            enclosed_toroidal_current = mu_0 / 2 / jnp.pi * current.compute(c_l, dr=0)
            term1 = data["psi_r"] / data["sqrt(g)"]
            term2 = data["g_tt"] * data["lambda_z"] - data["g_tz"] * (
                1 + data["lambda_t"]
            )
            num = surface_averages(
                grid, term1 * term2, match_grid=True, denominator=4 * jnp.pi ** 2
            )
            den = surface_averages(
                grid, term1 * data["g_tt"], match_grid=True, denominator=4 * jnp.pi ** 2
            )
            data["iota"] = (enclosed_toroidal_current + num) / den

        if check_derivs("iota_r", R_transform, Z_transform, L_transform):
            enclosed_toroidal_current_r = mu_0 / 2 / jnp.pi * current.compute(c_l, dr=1)
        ...
        if check_derivs("iota_rr", R_transform, Z_transform, L_transform):
            ...
            enclosed_toroidal_current_rr = (
                mu_0 / 2 / jnp.pi * current.compute(c_l, dr=2)
            )
            g_tt_rr = 2 * (
