...
from desc.compute.utils import compress, expand, surface_averages, surface_integrals
...
def compute_rotational_transform(
    ...
    **kwargs,
    ...
    current_from_solved_eq: ndarray
        Used for debugging.
    ...
    if iota is not None:
        ...
        surface_areas = surface_integrals(grid)
        ...
        if check_derivs("iota", R_transform, Z_transform, L_transform):
            ...
            num = surface_averages(grid, num, denominator=surface_areas)
            den = surface_averages(grid, den, denominator=surface_areas)
            # Δ(poloidal flux) generated by the toroidal current enclosed by the flux surface
            # is (mu_0 * I in amps) = 2pi * (I in tesla-meters)
            if current_from_solved_eq is None:
                enclosed_toroidal_current = mu_0 * compress(
                    grid, current.compute(c_l, dr=0)
                )
            else:
                # current_from_solved_eq is data["I"] at unique rho values of the tranform's grid
                enclosed_toroidal_current = 2 * jnp.pi * current_from_solved_eq
            iota = (enclosed_toroidal_current + num) / den
            data["iota"] = expand(grid, iota)

        if current_from_solved_eq is None and check_derivs(
            "iota_r", R_transform, Z_transform, L_transform
        ):
            ...
            num_r = surface_averages(grid, num_r, denominator=surface_areas)
            den_r = surface_averages(grid, den_r, denominator=surface_areas)
            enclosed_toroidal_current_r = mu_0 * compress(
                grid, current.compute(c_l, dr=1)
            )
            iota_r = (enclosed_toroidal_current_r + num_r - iota * den_r) / den
            data["iota_r"] = expand(grid, iota_r)
        ...
        if check_derivs("iota_rr", R_transform, Z_transform, L_transform):
            ...
            num_rr = surface_averages(grid, num_rr, denominator=surface_areas)
            den_rr = surface_averages(grid, den_rr, denominator=surface_areas)
            enclosed_toroidal_current_rr = mu_0 * compress(grid, current.compute(c_l, dr=2))
            iota_rr = (
            ...
            data["iota_rr"] = expand(grid, iota_rr)
            """
