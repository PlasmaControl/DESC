...
from .utils import compute_jac_scale
...
class LinearConstraintProjection(ObjectiveFunction):
    ...
    def build(self, eq, use_jit=None, verbose=1):
        ...
        (
            ...
            self._constraints, self._args, self._dimensions
        )
...
class ProximalProjection(ObjectiveFunction):
    ...
    def set_args(self, *args):
        ...
        self._other_args = [
            arg for arg in sorted(self._objective.args) if arg not in arg_order
        ]
    ...
    def _update_equilibrium(self, x, store=False):
        ...
        if np.any(
            [np.any(deltas[arg]) for arg in deltas if arg in self._constraint.args]
        ):
        ...
        if self._eq_threshold is not None:
        ...
        if store:
            ...
            for arg in arg_order + tuple(self._other_args):
                ...
                if arg in arg_order and val.size:
    ...
    def jac(self, x):
        ...
        Fx = align_jacobian(Fx, self._constraint, self._full_args, self._dimensions)
