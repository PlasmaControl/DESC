...
def format_bdry(M, N, NFP, bdry, in_mode, out_mode, ntheta=None, nphi=None):
    ...
    if in_mode == 'real' and out_mode == 'spectral':
        ...
        bdryM = bdryM.flatten()
        bdryN = bdryN.flatten()
        interp = np.stack([double_fourier_basis(
            bdry_theta, bdry_phi, m, n, NFP) for m, n in zip(bdryM, bdryN)]).T

        bR, bZ = np.linalg.lstsq(interp, np.array([bdryR, bdryZ]).T)[0].T
        return jnp.asarray(bdryM), jnp.assarray(bdryN), jnp.asarray(bR), jnp.asarray(bZ)
...
def compute_bc_err_four(cR, cZ, cL, zern_idx, lambda_idx, bdryR, bdryZ, bdryM, bdryN, NFP, sample=1.5):
    ...
    zern_bdry_interp = jnp.stack([fourzern(
        rho, vartheta, zeta, lmn[0], lmn[1], lmn[2], NFP, 0, 0, 0) for lmn in zern_idx]).T
    R = jnp.matmul(zern_bdry_interp, cR).flatten()
    Z = jnp.matmul(zern_bdry_interp, cZ).flatten()

    four_bdry_interp = jnp.stack([double_fourier_basis(
        bdry_theta, bdry_phi, m, n, NFP) for m, n in zip(bdryM, bdryN)]).T
    cRb, cZb = jnp.linalg.lstsq(
        four_bdry_interp, jnp.array([R, Z]).T, rcond=None)[0].T
