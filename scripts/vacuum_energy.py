...
if os.path.exists(save_path + phi_save_name):
    ...
    if pest:
        ...
        data_phi = field.compute(
            ["phi_matrix"], grid=pest_grid,
            problem="exterior Neumann", chunk_size=1,
        )
...
if pest:
    ...
    integration_weights = (
        sqrtg_grad_rho
        * grid0.meshgrid_reshape(grid0.weights, order="rtz").reshape(n_surf)
    )
else:
    data_eq = eq.compute(
...
def compute_external_coil_energy(coil_field, R0, a, L=50, M_quad=50, r_max_factor=20.0):
    ...
    B       = np.array(coil_field.compute_magnetic_field(
...
ax.plot(delta_h_fracs, W_V_vals,      marker="o",
...
np.savez(
    save_path + "vacuum_energy_scan.npz",
    delta_h_fracs=delta_h_fracs,
    W_V_vals=W_V_vals,
    W_V_true_vals=W_V_true_vals,
)
error = np.abs(W_V_vals - W_V_true_vals)/W_V_true_vals
print("Relative error between BIE and direct volume integral:")
print(error)
print("max error", error.max(), "min error", error.min())
