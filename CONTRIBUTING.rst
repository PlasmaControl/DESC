...
question!!! <#i-dont-want-to-read-this-whole-thing-i-just-have-a-question>`__
...
-  `Issue and Pull Request Labels <#issue-and-pull-request-labels>`__

.. _i-don't-want-to-read-this-whole-thing-i-just-have-a-question!!!:
...
.. _how-do-i-submit-a-(good)-bug-report?:
...
-  **Provide specific examples to demonstrate the steps**. Include links
   ...
   If you're providing snippets in the issue, use `Markdown code
...
-  **If you're reporting that DESC crashed**, include the python stack
   ...
   a `code
   block <https://help.github.com/articles/markdown-basics/#multiple-lines>`__,
   a `file
   attachment <https://help.github.com/articles/file-attachments-on-issues-and-pull-requests/>`__,
...
-  If the problem started happening recently, **can you reproduce the
   problem in an older version of DESC?** What's the most recent version
...
possible <#how-do-i-submit-a-good-enhancement-suggestion>`__, including
the steps that you imagine you would take if the feature you're
requesting existed.
...
-  **Check
   the**\ `documentation <https://desc-docs.readthedocs.io/en/latest/>`__
   for tips — you might discover that the enhancement is already
   available.
-  **Perform a**\ `cursory
   search <https://github.com/ddudt/DESC/issues?q=is%3Aopen+is%3Aissue+label%3Aenhancement>`__
...
followed the above steps and verified that an issue does not already
...
-  `Good first
   issues <https://github.com/ddudt/DESC/issues?q=is%3Aopen+is%3Aissue+label%3A%22good+first+issue%22>`__
   - issues which should only require a few lines of code, and a test or
   two.
-  `Help wanted
   issues <https://github.com/ddudt/DESC/issues?q=is%3Aopen+is%3Aissue+label%3A%22help+wanted%22>`__
...
-  `Follow the PEP8
   format <https://www.python.org/dev/peps/pep-0008/>`__
...
-  Make things modular. Focus on small functions that `do one thing and
   do it well <https://en.wikipedia.org/wiki/Unix_philosophy#Origin>`__,
...
-  ``jax.numpy`` is *almost* a drop in replacement for ``numpy``, but
   ...
   differences <https://jax.readthedocs.io/en/latest/notebooks/Common_Gotchas_in_JAX.html>`__.
