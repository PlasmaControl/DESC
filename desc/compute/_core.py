...
def compute_pressure(params, transforms, profiles, data=None, **kwargs):
    ...
    if has_dependencies("p", params, transforms, profiles, data):
        data["p"] = profiles["pressure"].compute(params["p_l"])
    if has_dependencies("p_r", params, transforms, profiles, data):
        data["p_r"] = profiles["pressure"].compute(params["p_l"], dr=1)
    if has_dependencies("p_t", params, transforms, profiles, data):
        data["p_t"] = profiles["pressure"].compute(params["p_l"], dt=1)
    if has_dependencies("p_z", params, transforms, profiles, data):
        data["p_z"] = profiles["pressure"].compute(params["p_l"], dz=1)

    return data


def compute_pressure_anisotropy(params, transforms, profiles, data=None, **kwargs):
    """Compute anistropic pressure term d = (p_|| - p_perp)/B^2."""
    if data is None:
        data = {}

    if has_dependencies("d", params, transforms, profiles, data):
        data["d"] = profiles["anisotropy"].compute(params["d_lmn"])
    if has_dependencies("d_r", params, transforms, profiles, data):
        data["d_r"] = profiles["anisotropy"].compute(params["d_lmn"], dr=1)
    if has_dependencies("d_t", params, transforms, profiles, data):
        data["d_t"] = profiles["anisotropy"].compute(params["d_lmn"], dt=1)
    if has_dependencies("d_z", params, transforms, profiles, data):
