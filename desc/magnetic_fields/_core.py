...
def biot_savart_general_surface_divergence(
    re,
    e_sub_theta,
    e_sub_zeta,
    e_sup_theta_s,
    ...
    re, rs, J, dV, e_sub_theta, e_sub_zeta, e_sup_theta_s, e_sup_zeta_s = map(
        jnp.asarray,
        (re, rs, J, dV, e_sub_theta, e_sub_zeta, e_sup_theta_s, e_sup_zeta_s),
    )
        ...
        num = jnp.sum(
            ...
            * (
                jnp.cross(
                    ...
                    (
                        d**2 * e_sub_theta.T
                        - 3 * jnp.sum(r * e_sub_theta, axis=-1) * r.T
                    ).T,
                    axis=-1,
                )
                + jnp.cross(
                    e_sup_zeta_s,
                    (
                        d**2 * e_sub_zeta.T - 3 * jnp.sum(r * e_sub_zeta, axis=-1) * r.T
                    ).T,
                    axis=-1,
                )
            ),
