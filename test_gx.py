...
#idx_Zcs = eq_init.surface.Z_basis.get_idx(M=1, N=0)

# boundary modes to constrain
R_modes = np.delete(eq_init.surface.R_basis.modes, [idx_Rcc], axis=0)
Z_modes = np.delete(eq_init.surface.Z_basis.modes, [idx_Zsc], axis=0)
...
objective_fT = ObjectiveFunction(GXWrapper(target=0.1),verbose=0,use_jit=False)
