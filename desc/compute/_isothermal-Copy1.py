...
def H2(params, transforms, profiles, data, **kwargs):
    ...
    data["H_2"] = jnp.cross(data["n_rho"], data["H_1"], axis=-1)
...
def H1(params, transforms, profiles, data, **kwargs):
    ...
    phi = find_phi(data,2*(transforms["grid"].N)+1)
    grad = grad_(phi,data,2*(transforms["grid"].N)+1)
    #data["H_1"] = data["e^theta_s"] - grad
    data["H_1"] = data["e^zeta_s"] - grad
...
def find_phi(data,n_size):
    
    x = jnp.ones(data["theta"].shape[0])
    fun_wrapped = lambda x: u_div_residual(x,data,n_size)
    A_ = Derivative(fun_wrapped,deriv_mode="looped").compute(x)
    #A_ = jax.jacfwd(fun_wrapped)(x)
    
    return jnp.linalg.pinv(A_)@data["nabla_s^2_zeta"]

def u_div_residual(y,data,n_size,):
#def u_div_residual(y,data,):

    f_t = first_derivative_t(y, data,n_size,)
    f_z = first_derivative_z(y, data,n_size,)
    f_tt = first_derivative_t(f_t, data,n_size,)
    f_zz = first_derivative_z(f_z, data,n_size,)
    f_tz = first_derivative_z(f_t, data,n_size,)
...
def grad_(y,data,n_size):
    
    f_t_ = first_derivative_t(y, data,n_size,)
    f_z_ = first_derivative_z(y, data,n_size,)
...
def first_derivative_t(a_mn,data,m_size,n_size):
    ...
    # Rearrange A as a matrix
    A2 = a_mn.reshape((m_size, n_size)).T
    
    # theta-step
    dt = data["theta"][1] - data["theta"][0]
    
    # d(V)/dz
    A_t = jnp.zeros_like(A2)
    # at i = 0
    A_t = A_t.at[:, 0].set( (A2[:, 1] - A2[:, m_size - 1]) * (2 * dt) ** (-1) )
    # at i = n_size
    A_t = A_t.at[:, m_size - 1].set((A2[:, 0] - A2[:, m_size - 2]) * (2 * dt) ** (-1) )
    # Intermediate steps
    A_t = A_t.at[:, 1:m_size - 1].set((A2[:, 2:m_size] - A2[:, 0:m_size - 2]) * (2 * dt) ** (-1) )
...
def first_derivative_z(a_mn,data,m_size,n_size):
    ...
    # Rearrange A as a matrix
    A1 = a_mn.reshape((m_size, n_size)).T
    
    # dz-step
    dz = data["zeta"][m_size] - data["zeta"][0]
    
    # d(sigma)/dt
    A_z = jnp.zeros_like(A1)
    # i = 0
    A_z = A_z.at[0, :].set( (A1[1, :] - A1[n_size-1, :]) * (2 * dz) ** (-1) )
    # i = n_size
    A_z = A_z.at[n_size-1, :].set( (A1[0, :] - A1[n_size-2, :]) * (2 * dz) ** (-1) )
    # Intermediate steps
    A_z = A_z.at[1:n_size - 1, :].set((A1[2:n_size, :] - A1[0:n_size - 2, :]) * (2 * dz) ** (-1))
