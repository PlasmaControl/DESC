...
from desc.compute.utils import compress, expand, surface_averages
    ...
    **kwargs,
    ...
    if iota is not None:
        ...
        if check_derivs("iota", R_transform, Z_transform, L_transform):
            ...
            num = surface_averages(grid, num, denominator=4 * jnp.pi ** 2)
            den = surface_averages(grid, den, denominator=4 * jnp.pi ** 2)
        ...
        if current_from_solved_eq is None and check_derivs(
            ...
            num_r = surface_averages(grid, num_r, denominator=4 * jnp.pi ** 2)
            den_r = surface_averages(grid, den_r, denominator=4 * jnp.pi ** 2)
        ...
        if check_derivs("iota_rr", R_transform, Z_transform, L_transform):
            ...
            num_rr = surface_averages(grid, num_rr, denominator=4 * jnp.pi ** 2)
            den_rr = surface_averages(grid, den_rr, denominator=4 * jnp.pi ** 2)
