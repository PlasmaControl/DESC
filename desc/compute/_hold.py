...
@register_compute_fun(
    name="Gamma_c Integrand",
    label="\\gamma_c = \\arctan\\left(\\frac{\\text{radial drift}}{\\text{poloidal drift}}\\right)",
    units="~",
    description="Integrand for fast ion confinement proxy Γ_c in Velasco et al.",
    dim=2,
    params=[],
    transforms={"grid": []},
    profiles=[],
    coordinates="r",
    data=["cvdrift0", "gbdrift (periodic)", "gbdrift (secular)/phi"],
    resolution_requirement="tz",
    grid_requirement={"can_fft2": True},
)
def _Gamma_c_Velasco_integrand(params, transforms, profiles, data, **kwargs):
    """
    This function computes the ratio of radial to poloidal drift at a given pitch (eq14 Velasco).

    computes 2/pi * (bounceavg[radial_drift]/ bounceavg[poloidal_drift])


    uses gammac^*

    Returns:
    gamma_c: The drift ratio γ_c = arctan(radial_drift / poloidal_drift).
    v_tau: The transit velocity used for weighting in Γ_c.
    ...
    bounce = Bounce2D(  # object to handle bounce integrals
        transforms["grid"],  # grid integration occurs on
        data,
        data["theta"],  # poloidal angles for bounce integration
        kwargs.get("Y_B", data["theta"].shape[-1] * 2),  # resolution for bounce quad
        kwargs.get("alpha", jnp.array([0.0])),  # field lines
        kwargs.get("num_transit", 20),  # num torodial transits
        kwargs.get("quad"),  # quad rule for integration
        is_fourier=True,
        spline=kwargs.get("spline", True),  # use spline interpolation
    )

    # compute bounce averaged values of transit velocity, radial drift, poloidal drift
    ...
    def fun(pitch_inv):
        ...
        v_tau, radial_drift, poloidal_drift = bounce.integrate(
            [
                ...
                _radial_drift,
                _poloidal_drift,
            ],  # functions to be bounce integrated
            pitch_inv,  # changed to pass an array of pitch_inv
            data,
            ["cvdrift0", "gbdrift (periodic)", "gbdrift (secular)/phi"],  # drift terms
            bounce.points(pitch_inv, num_well),  # bounce points (dept on pitch, wells)
            is_fourier=True,
        )

        gamma_c = jnp.arctan(
            safediv(radial_drift, poloidal_drift)
        )  # one gamma_c^* is this ratio of bounce avged

        return gamma_c, v_tau

    gamma_c, v_tau = batch_map(fun, data["pitch_inv"], pitch_batch_size)

    data["gamma_c"] = gamma_c
    data["v_tau"] = v_tau
