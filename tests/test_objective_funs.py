...
def test_bd_min_objective():
    ...
    obj1 = BDistanceVariance(
        winding_surf1, eq=eq, normalize=False, plasma_grid=grid, surface_grid=grid
    )
    obj2 = BDistanceVariance(
        winding_surf2, eq=eq, normalize=False, plasma_grid=grid, surface_grid=grid
    )
    obj1.build()
    obj2.build()
    ...
    with pytest.warns(UserWarning):
        obj2.build()
    L1 = obj1.compute_scaled(*obj1.xs(eq))
