...
@pytest.mark.parametrize("name, X, Y", [("NCSX", 82, 59), ("HELIOTRON", 50, 30)])
...
def test_plot_bavg_drift():
    ...
    plt.savefig("raddrift_contour_plot_1.pdf")
    plt.close()
    ...
    plt.savefig("raddrift_contour_plot_2.pdf")
    plt.close()
...
def test_plot_binormal_drift():
    ...
    from tests.test_integrals import TestBounce, TestBounce2D
    ...
    fig = bounce.check_points(
        (points[0][..., 5::10, :], points[1][..., 5::10, :]),
        pitch_inv[5::10],
        plot=True,
    ...
    ax = plt.gca()
    ax.xaxis = pi_formatter(ax.xaxis)
    ax.set_yticks(np.linspace(0.95, 1.025, 4))
    for spine in ax.spines.values():
        spine.set_visible(True)
        spine.set_linewidth(2)
