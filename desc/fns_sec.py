...
def eqn_residual(s_basis,
    ...
    # Define sigma
    s = (y[s_basis.num_modes + V_basis.num_modes]*data["theta"]
         + y[s_basis.num_modes + V_basis.num_modes + 1]*data["zeta"]
         + fs["s"])
    
    # Define nabla_gamma_V
    nabla_gamma_V = ((y[s_basis.num_modes + V_basis.num_modes + 2]*data["theta"]**(0) 
                      + fs["V_t"]
    ...
    nabla_gamma_s = ((y[s_basis.num_modes + V_basis.num_modes]*data["theta"]**(0) 
                      + fs["s_t"]
                     )*data["e^theta_gamma"].T 
    ...
    nabla_gamma2_V = (y[s_basis.num_modes + V_basis.num_modes+2]*data["nabla_gamma^2_theta"]
                      + y[s_basis.num_modes + V_basis.num_modes+3]*data["nabla_gamma^2_zeta"]
    ...
    cost = jnp.concatenate([Bn_error,nabla_gamma_dot_K,
                            #mag_K
                           ])
...
def sol_eval(y,
    ...
    # Define V
    V = (y[s_basis.num_modes + V_basis.num_modes + 2]*data["theta"]
