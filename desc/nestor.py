...
def eval_axis_geometry(R_lmn, Z_lmn, Ra_transform, Za_transform, nzeta, NFP):
    ...
    axis = jnp.array([raxis*jnp.cos(phiaxis),
                     raxis*jnp.sin(phiaxis),
                     zaxis])
...
def biot_savart(eval_pts, coil_pts, current):
    ...
    Rf = jnp.linalg.norm(eval_pts[:, :, jnp.newaxis] - coil_pts[:,jnp.newaxis,1:], axis=0)
    Ri_p_Rf = Ri + Rf
...
def modelNetToroidalCurrent(axis, current, coords, normal):
    ...
    axis = jnp.array([axis["R"]*jnp.cos(axis["phi"]),
                     axis["R"]*jnp.sin(axis["phi"]),
                     axis["Z"]])
...
def compute_analytic_integrals(normal, jacobian, TS, B_field, mf, nf, ntheta, nzeta, cmns, weights):
    ...
    m,n = jnp.meshgrid(jnp.arange(mf+1), jnp.concatenate([jnp.arange(nf+1), jnp.arange(-nf,0)]), indexing="ij")

    I_mn = jnp.zeros([mf+1, 2*nf+1])
    I_mn = jnp.where(jnp.logical_or(m==0,n==0), (n>=0)*jnp.sum(cmns[:,m,n] * (ft_T_p[:, m, n].imag + ft_T_m[:, m, n].imag), axis=0), I_mn)
    I_mn = jnp.where(jnp.logical_and(m != 0, n>0), jnp.sum(cmns[:,m,n] * ft_T_p[:, m, n].imag, axis=0), I_mn)
    I_mn = jnp.where(jnp.logical_and(m != 0, n<0), jnp.sum(cmns[:,m,-n] * ft_T_m[:, m, n].imag, axis=0), I_mn)                

    K_mntz = jnp.zeros([mf+1, 2*nf+1, ntheta_sym*nzeta])
    K_mntz = jnp.where(jnp.logical_or(m==0,n==0)[:,:,jnp.newaxis], jnp.sum(cmns[:,m,n, jnp.newaxis] * (ft_S_p[:, m, n, :].imag + ft_S_m[:, m, n, :].imag), axis=0), K_mntz)
    K_mntz = jnp.where(jnp.logical_and(m!=0,n>0)[:,:,jnp.newaxis], jnp.sum(cmns[:,m,n, jnp.newaxis] * ft_S_p[:, m, n, :].imag,  axis=0), K_mntz)
    K_mntz = jnp.where(jnp.logical_and(m!=0,n<0)[:,:,jnp.newaxis], jnp.sum(cmns[:,m,-n, jnp.newaxis] * ft_S_m[:, m, n, :].imag, axis=0), K_mntz)
...
def regularizedFourierTransforms(coords, normal, jacobian, B_field, tan_theta, tan_zeta, mf, nf, ntheta, nzeta, NFP, weights):
    ...
    kt_ip, kz_ip, kt_i, kz_i = jnp.meshgrid(jnp.arange(ntheta_sym),
                                           jnp.arange(nzeta),
                                           jnp.arange(ntheta),
    ...
    ftemp = (gsave[:,:,:,:,jnp.newaxis]
             - 2*X_full*coords["X"].reshape((-1, nzeta))[jnp.newaxis,jnp.newaxis,:,:,jnp.newaxis]
             - 2*Y_full*coords["Y"].reshape((-1, nzeta))[jnp.newaxis,jnp.newaxis,:,:,jnp.newaxis])
    ftemp = 1/jnp.where(ftemp<=0, 1, ftemp)
    htemp = jnp.sqrt(ftemp)
    gtemp = (  coords["X"].reshape((-1, nzeta))[jnp.newaxis,jnp.newaxis:,:,jnp.newaxis]*X_n
             + coords["Y"].reshape((-1, nzeta))[jnp.newaxis,jnp.newaxis:,:,jnp.newaxis]*Y_n
             + dsave[:,:,:,:,jnp.newaxis])
    ...
    g_mntz = jnp.concatenate([g_tzmn[:ntheta_sym, :nzeta, :mf+1, :nf+1].imag,
    ...
    bexni = -weights * B_field["Bn"] * 4.0*jnp.pi*jnp.pi        
    h_tz = jnp.sum(bexni.reshape((ntheta_sym, nzeta,1,1)) * source[:ntheta_sym, :, :, :], axis=(0,1))
    ...
    h_mn = jnp.concatenate([h_mn[:mf+1,:nf+1].imag, h_mn[:mf+1,-nf:].imag], axis=1)
...
def compute_scalar_magnetic_potential(I_mn, K_mntz, g_mntz, h_mn, mf, nf, ntheta, nzeta, weights):
    ...
    amatrix_4d = jnp.concatenate([g_mnmn[:, :, :mf+1, :nf+1].imag, g_mnmn[:, :, :mf+1, -nf:].imag], axis=-1)
    ...
    amatrix_4d = jnp.where(jnp.logical_and(m==0, n>nf)[:,:,jnp.newaxis, jnp.newaxis], 0, amatrix_4d)
    # add diagnonal terms (#TODO: why 4*pi^3 instead of 1 ?)         
    amatrix_4d = put(amatrix_4d, Index[m, n, m, n], amatrix_4d[m,n,m,n] + 4.0*jnp.pi**3)
