...
- Adds ``a(z)``, the effective minor radius ``sqrt(A(zeta)/pi)`` of the enclosed cross-section, for ``Equilibrium``, ``FourierRZToroidalSurface`` and ``ZernikeRZToroidalSection``.
- Documents that the constant-zeta cross-section family (``A``, ``A(z)``, ``a``, ``a(z)``, ``R0/a``, ``a_major/a_minor``, ``perimeter(z)``) can be badly wrong with a generalized toroidal angle, since a constant-zeta surface is then not planar. Measured 24 % on ``A`` and 33 % on elongation for a racetrack with ``|W_lmn| = 0.53``; the error grows with the shaping, so it is worst for an elongated or high-torsion magnetic axis. The note states the workaround (a ``LinearGrid`` with ``sym=False`` and ``override_grid=False``, which selects the line-integral branch).
...
- Fixes ``solve_continuation_automatic`` for equilibria with a generalized toroidal angle (``omega != 0``). ``surf_axisym`` was built with a positional ``change_resolution(L, M, 0)``, and ``FourierRZToroidalSurface.change_resolution`` defaults ``Mz``/``Nz`` to the surface's *current* values, so the ``omega`` basis was left untouched. ``get_deltas`` then saw identical ``W_lmn`` and emitted no ``Wb_lmn`` at all, leaving ``omega`` at full strength from the first step while ``R``, ``Z`` ramped from zero. ``Mz=0, Nz=0`` are now passed explicitly, and ``Wb_lmn`` is scaled by ``bdry_step`` alongside ``Rb_lmn``/``Zb_lmn``.
- ``plot_section`` now verifies the ``phi -> zeta`` inversion for ``omega != 0`` equilibria. The inversion is a Newton solve started from ``zeta = phi``, which on a strongly shaped device can converge to a different branch of the map; those points were previously drawn as spurious features far outside the plasma and corrupted any maximum taken over the plotted field. Failures are now masked and warned about. See ``_find_failed_phi_inversion`` for the remaining work (a bracketed solve, since ``phi(zeta)`` is monotonic wherever the chart is valid).
- Fixes ``BoundaryError`` with a sheet current for equilibria with a generalized toroidal angle (``omega != 0``). The sheet current surface's parameters are assembled by hand from a fixed list of keys, which predates ``omega``, so computing ``K`` on that surface raised ``KeyError: 'W_lmn'``. The boundary's ``Wb_lmn`` is now passed through, mirroring how ``R_lmn`` takes ``Rb_lmn``. No effect when ``omega = 0``, where the basis is empty.
- Fixes exponential spectral scaling (``x_scale="ess"``) for ``omega``. ``FourierRZCurve`` now scales ``W_n``, and ``Equilibrium`` scales ``W_lmn`` and inherits ``Wa_n``/``Wb_lmn`` from the axis and boundary, as is already done for ``R``, ``Z`` and ``lambda``. Previously these fell through to the all-ones scale from the base class while ``R``, ``Z`` and ``lambda`` were scaled, so ``omega`` was silently excluded from the scaling it asked for. No effect when ``omega = 0``.
- Fixes the ``phi -> zeta`` inversion in ``plot_section`` for equilibria with a generalized toroidal angle. It was a Newton solve started from ``zeta = phi``, which on a strongly shaped device converged to a different branch for ~5 % of points (measured 334 of 7098 on a racetrack); those were previously masked, leaving holes in the section. Since ``rho`` and ``theta`` are the same labels in both bases the inversion is one dimensional per node, and a valid chart guarantees ``dphi/dzeta > 0`` and ``|omega| < pi``, so the root is unique and bracketed by ``[phi - pi, phi + pi]``. It is now found by bisection, which cannot reach the wrong branch: 0 of 7098 failures on the same case, and no holes. The verification is kept as a safety net.
- Fixes ``test_solve_with_x_scale``, which built an ``x_scale`` vector from a hardcoded list of parameter sizes predating ``omega``, so it was three entries short (156 vs 159) once ``W_lmn``, ``Wa_n`` and ``Wb_lmn`` became optimizable parameters.
