...
def compute_magnetic_field_magnitude(Psi, R0_n, Z0_n, r_lmn, l_lmn, p_l, i_l,
    ...
    magnetic_field['|B|'] = jnp.sqrt(
        ...
        magnetic_field['B^zeta']**2*dot(cov_basis['e_zeta'], cov_basis['e_zeta'], 0) + \
        2*magnetic_field['B^theta']*magnetic_field['B^zeta']*dot(cov_basis['e_theta'], cov_basis['e_zeta'], 0))
...
def compute_current_density(Psi, R0_n, Z0_n, r_lmn, l_lmn, p_l, i_l,
    ...
    jacobian['g_t'] = dot(cov_basis['e_rho_t'],
                      cross(cov_basis['e_theta'], cov_basis['e_zeta'], 0), 0) \
                    + dot(cov_basis['e_rho'],
    ...
    magnetic_field['B^theta_z'] = magnetic_field['B0_z']*(profiles['iota'] - polar_coords['lambda_z']) \
                                - magnetic_field['B0']*polar_coords['lambda_zz']
    magnetic_field['B^zeta_r']  = magnetic_field['B0_r']*(1 + polar_coords['lambda_t']) \
                                + magnetic_field['B0']*polar_coords['lambda_rt']
    magnetic_field['B^zeta_t']  = magnetic_field['B0_t']*(1 + polar_coords['lambda_t']) \
                                + magnetic_field['B0']*polar_coords['lambda_tt']
    magnetic_field['B^zeta_z']  = magnetic_field['B0_z']*(1 + polar_coords['lambda_t']) \
                                + magnetic_field['B0']*polar_coords['lambda_tz']
    ...
    current_density['J^rho']   = (magnetic_field['B_zeta_t']  - magnetic_field['B_theta_z']) \
                               / (mu0*jacobian['g'])
    current_density['J^theta'] = (magnetic_field['B_rho_z']   - magnetic_field['B_zeta_r']) \
                               / (mu0*jacobian['g'])
    current_density['J^zeta']  = (magnetic_field['B_theta_r'] - magnetic_field['B_rho_t']) \
                               / (mu0*jacobian['g'])
...
def compute_force_error_magnitude(Psi, R0_n, Z0_n, r_lmn, l_lmn, p_l, i_l,
    ...
    force_error['|beta|'] = jnp.sqrt(
        magnetic_field['B^zeta']**2*dot(con_basis['e^theta'], con_basis['e^theta'], 0) + \
        magnetic_field['B^theta']**2*dot(con_basis['e^zeta'], con_basis['e^zeta'], 0) - \
        2*magnetic_field['B^theta']*magnetic_field['B^zeta']*dot(con_basis['e^theta'], con_basis['e^zeta'], 0))

    force_error['|F|'] = jnp.sqrt(
        force_error['F_rho']**2*dot(con_basis['e^rho'], con_basis['e^rho'], 0) + \
        force_error['F_beta']**2*force_error['|beta|']**2)

    return force_error, current_density, magnetic_field, profiles, con_basis, jacobian, cov_basis, toroidal_coords, polar_coords
