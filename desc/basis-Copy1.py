...
class DoubleChebyshevSeries(_Basis):
    ...
    def evaluate(
        ...
        poloidal = chebyshev(t[:, np.newaxis], m, 
                             #1, 
                             derivatives[1])
        toroidal = chebyshev(z[:, np.newaxis], n, 
                             #self.NFP, 
                             derivatives[2])
        if unique:
            poloidal = poloidal[toutidx][:, moutidx]
...
def chebyshev(r, l, dr=0):
    ...
    x = 2 * r/(2*np.pi) - 1  # shift
    if dr == 0:
        return jnp.cos(l * jnp.arccos(x))
    if dr == 1:
        # dy/dr = dy/dx * dx/dr = dy/dx * 2
        
        #2*np.concatenate([-(l)**2*jnp.cos(l * np.pi)],
        #                         (l*jnp.sin(l*jnp.arccos(x[1:-1])))*(1-x[1:-1]**2)**(-1/2),
        #                         [l**2])
        
        return  2*jnp.stack([-l**2*jnp.cos(l * jnp.pi),
                           (l*jnp.sin(l*jnp.arccos(x[1:-1])))*(1-x[1:-1]**2)**(-1/2),
                           l**2]).T
    
    if dr == 2:
        #4*np.concatenate([l**2*(l**2-1)/3*jnp.cos(l*np.pi)],
        #                         (-l**(2)*jnp.cos(l*x[1:-1])*(1-x[1:-1]**2)**(-1)
        #                          + l*x[1:-1]*jnp.sin(l*jnp.arccos(x[1:-1]))*(1-x[1:-1]**2)**(-3/2)
        #                         ),
        #                         [l**2*(l**2-1)/3])
        return  4*jnp.stack([l**2*(l**2-1)/3*jnp.cos(l*jnp.pi),
                            (-l**(2)*jnp.cos(l*x[1:-1])*(1-x[1:-1]**2)**(-1)
                             + l*x[1:-1]*jnp.sin(l*jnp.arccos(x[1:-1]))*(1-x[1:-1]**2)**(-3/2)
                            ),
                            l**2*(l**2-1)/3]).T
