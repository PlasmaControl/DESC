...
def K_dips(p_M, p_N, sdata, sgrid, surface, y, dt, dz, N, d_0):
    ...
    for key in ["u_iso", "v_iso"]:
        assert isinstance(du_data[key], jnp.ndarray), f"du_data[{key}] is not a JAX array: {type(du_data[key])}"
        assert isinstance(dd_data[key], jnp.ndarray), f"dd_data[{key}] is not a JAX array: {type(dd_data[key])}"
        assert isinstance(dr_data[key], jnp.ndarray), f"dr_data[{key}] is not a JAX array: {type(dr_data[key])}"
        assert isinstance(dl_data[key], jnp.ndarray), f"dl_data[{key}] is not a JAX array: {type(dl_data[key])}"
    ...
    indices_per_device = (r + num_devices - 1) // num_devices
    indices_split = []
    for i in range(num_devices):
        start = i * indices_per_device
        end = min((i + 1) * indices_per_device, r)
        chunk = indices[start:end]
        padded_chunk = jnp.pad(chunk, (0, indices_per_device - len(chunk)), 
                              mode='constant', constant_values=-1)
        indices_split.append(padded_chunk)

    indices_array = jnp.array(indices_split)
    ...
    def safe_omega_wrapper(i, y, sdata, du_data, dd_data, dr_data, dl_data, N, d_0, r):
        ...
        def false_fun():
            # Use sdata["n_rho"].shape[0] to match the output shape of omega_wrapper_jax
    ...
    parallel_wrapper = pmap(lambda idx: vmap(lambda i: safe_omega_wrapper(
        i, y, sdata, du_data, dd_data, dr_data, dl_data, N, d_0, r))(idx))

    results = parallel_wrapper(indices_array)
    omega_total_real = jnp.sum(jnp.concatenate(results[0]), axis=0)
    omega_total_imag = jnp.sum(jnp.concatenate(results[1]), axis=0)

    return ((sdata["lambda_iso"] ** (-1)) * (
            omega_total_imag * -cross(sdata["n_rho"], sdata["e_u"]).T 
