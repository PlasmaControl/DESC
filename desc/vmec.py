...
def vmec_error(equil, vmec_data, Npol=8, Ntor=8):
    ...
    R_desc = zernike_transform.transform(equil['cR'], 0, 0, 0).reshape((ns, Npol, Ntor))
    Z_desc = zernike_transform.transform(equil['cZ'], 0, 0, 0).reshape((ns, Npol, Ntor))
    ...
    for k in range(Ntor):           # toroidal angle
        for i in range(ns):         # flux surface
            ...
            for j in range(Npol):   # poloidal angle
                ...
                f2pi = sfl_err(np.array([2*np.pi]), vartheta[j], zeta[k], vmec_data, i)
            ...
            R_vmec[i,:,k] = vmec_transf(vmec_data['rmnc'][i,:], vmec_data['xm'], vmec_data['xn'], theta, phi[k], trig='cos').flatten()
            Z_vmec[i,:,k] = vmec_transf(vmec_data['zmns'][i,:], vmec_data['xm'], vmec_data['xn'], theta, phi[k], trig='sin').flatten()
            if not vmec_data['sym']:
                R_vmec[i,:,k] += vmec_transf(vmec_data['rmns'][i,:], vmec_data['xm'], vmec_data['xn'], theta, phi[k], trig='sin').flatten()
                Z_vmec[i,:,k] += vmec_transf(vmec_data['zmnc'][i,:], vmec_data['xm'], vmec_data['xn'], theta, phi[k], trig='cos').flatten()
