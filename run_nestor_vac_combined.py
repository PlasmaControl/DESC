...
class BoundaryErrorNESTOR(_Objective):
    ...
    def build(self, eq, use_jit=True, verbose=1):
        ...
        self.ntheta = 2 * eq.M_grid + 1 if self.ntheta is None else self.ntheta
        self.nzeta = 2 * eq.N_grid + 1 if self.nzeta is None else self.nzeta
...
eq1.save("run_nestor_vac_combined_out1.h5")
with open("run_nestor_vac_combined_out1.pkl", "wb+") as f:
    pickle.dump(out, f)
...
veq.save("run_nestor_vac_combined_outv.h5")
with open("run_nestor_vac_combined_outv.pkl", "wb+") as f:
    pickle.dump(out, f)
