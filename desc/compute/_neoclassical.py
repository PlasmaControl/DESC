...
def _dJpar_ds_num(data, B, pitch):
...
def _dJpar_dalpha_num(data, B, pitch):
    """Integrand of numerator of bounce averaged normal drift."""
    g = jnp.sqrt(jnp.abs(1 - pitch * B))
    return +(0.5 * g * data["cvdrift0"]) + (0.5 * data["cvdrift0"] / g)
...
def _Jpar(params, transforms, profiles, data, **kwargs):
    ...
    def Jpar(data):
        ...
        def fun(pitch_inv):
            Jpar_num, v_tau = bounce.integrate(
                [_Jpar_num, _v_tau],
                pitch_inv,
                data,
            ...
            return safediv(Jpar_num, v_tau).sum(axis=-1)
...
def _dJpar_dalpha(params, transforms, profiles, data, **kwargs):
    ...
    def dJpar_ds(data):
        ...
        def fun(pitch_inv):
            dJpar_dalpha, v_tau = bounce.integrate(
                [_dJpar_ds, _v_tau],
                pitch_inv,
                data,
                ["cvdrift", "gbdrift"],
