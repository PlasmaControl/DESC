...
class ChebyshevRZToroidalSurface(Surface):
    ...
    @property
    def rho(self):
        """Surface label (normalized radius)."""
    ...
    def R_lmn(self, new):
        if len(new) == self.R_basis.num_modes:
            ...
            raise ValueError(
                f"R_lmn {len(new)} doesn't match basis w {self.R_basis.num_modes} modes"
            )
    ...
    def Z_lmn(self, new):
        if len(new) == self.Z_basis.num_modes:
            ...
            raise ValueError(
                f"Z_lmn {len(new)} doesn't match basis w {self.Z_basis.num_modes} modes"
            )
