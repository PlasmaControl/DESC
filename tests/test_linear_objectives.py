...
def test_LambdaGauge_sym(DummyStellarator):
    ...
    # RG: Omega is to phi what Lambda is to theta
    omega_con = FixOmegaGauge(eq)
    omega_con.build()

    # RG: I think this should hold for stellarator sym cases
    assert omega_con._params["W_lmn"].size == 0
...
def test_fixed_axis_and_theta_SFL_solve():
    ...
    constraints = (
        FixThetaSFL(eq=eq),
        FixZetaSFL(eq=eq),
    ) + get_NAE_constraints(eq, None, order=0, fix_lambda=False)
